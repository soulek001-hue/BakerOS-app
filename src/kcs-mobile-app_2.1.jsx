              ...
              {PRODS_S.map(p => {
                ...
                return (
                  <div key={p.id}>
                    ...
                    {sel && (
                      <div style={{marginTop:4, padding:"11px 13px", background:`color-mix(in srgb,${b.theme.primary} 4%,var(--bg))`, borderRadius:"0 0 12px 12px", border:`1px solid ${b.theme.primary}20`, borderTop:"none"}}>
                        {/* Quantity row */}
                        <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8}}>
                          <div>
                            <div style={{fontSize:12, fontWeight:700, color:"var(--tx)"}}>Quantity</div>
                            {p.sellBy && p.sellBy !== "each" && (
                              <div style={{fontSize:10, color:"var(--mu)", marginTop:1}}>
                                Sold by: {p.sellBy === "half_dozen" ? "Half Dozen (6)" : "Dozen (12)"}
                              </div>
                            )}
                            {p.minOrder && Number(p.minOrder) > 1 && (
                              <div style={{fontSize:10, color:b.theme.primary, fontWeight:700, marginTop:2}}>
                                ⚠ Minimum order: {p.minOrder} {p.sellBy === "half_dozen" ? "half dozens" : p.sellBy === "dozen" ? "dozens" : ""}
                              </div>
                            )}
                          </div>
                          {/* − qty + stepper */}
                          <div style={{display:"flex", alignItems:"center", gap:0, background:"var(--sf)", borderRadius:11, border:"1px solid var(--bd)", overflow:"hidden"}}>
                            <button
                              onClick={e => {
                                e.stopPropagation();
                                const min = Number(p.minOrder) || 1;
                                const current = sel.qty || min;
                                if (current > min) updateItem(p.id, "qty", current - 1);
                              }}
                              style={{width:36, height:36, border:"none", background:"transparent", fontSize:18, fontWeight:700, color:"var(--mu)", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center"}}
                            >−</button>
                            <div style={{width:40, textAlign:"center", fontSize:15, fontWeight:800, color:"var(--tx)"}}>
                              {sel.qty || Number(p.minOrder) || 1}
                            </div>
                            <button
                              onClick={e => {
                                e.stopPropagation();
                                const current = sel.qty || Number(p.minOrder) || 1;
                                updateItem(p.id, "qty", current + 1);
                              }}
                              style={{width:36, height:36, border:"none", background:"transparent", fontSize:18, fontWeight:700, color:b.theme.primary, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center"}}
                            >+</button>
                          </div>
                        </div>
                        ...
                        <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", padding:"7px 10px", background:"var(--bg)", borderRadius:9, marginBottom:10, fontSize:12}}>
                          <span style={{color:"var(--mu)"}}>{sel.qty || Number(p.minOrder) || 1} × ${p.price}</span>
                          <span style={{fontWeight:800, color:b.theme.primary}}>${((sel.qty || Number(p.minOrder) || 1) * p.price).toFixed(2)}</span>
                        </div>
                        ...
                        {sel.imageURL ? (
                          ...
                          <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:8}}>
                            <button onClick={e => { e.stopPropagation(); imgRefs.current[`gallery_${p.id}`]?.click(); }} style={{padding:"9px 8px", borderRadius:10, border:`1.5px dashed ${b.theme.primary}60`, background:"transparent", cursor:"pointer", fontFamily:"var(--fb)", fontSize:12, fontWeight:700, color:b.theme.primary, display:"flex", alignItems:"center", justifyContent:"center", gap:6}}>🖼 Gallery</button>
                            <button onClick={e => { e.stopPropagation(); imgRefs.current[`camera_${p.id}`]?.click(); }} style={{padding:"9px 8px", borderRadius:10, border:`1.5px dashed ${b.theme.primary}60`, background:"transparent", cursor:"pointer", fontFamily:"var(--fb)", fontSize:12, fontWeight:700, color:b.theme.primary, display:"flex", alignItems:"center", justifyContent:"center", gap:6}}>📷 Camera</button>
                          </div>
                        )}
                        <input ref={el => imgRefs.current[`gallery_${p.id}`] = el} type="file" accept="image/*" style={{display:"none"}} onChange={e => handleImage(p.id, e.target.files[0])}/>
                        <input ref={el => imgRefs.current[`camera_${p.id}`] = el} type="file" accept="image/*" capture="environment" style={{display:"none"}} onChange={e => handleImage(p.id, e.target.files[0])}/>
