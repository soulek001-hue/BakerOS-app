...
export default function App() {
  ...
  // ── Supabase DB sync ──────────────────────────────────────────────────────
  // Pull all baker data from Supabase on login and hydrate local state
  const hydrateFromSupabase = useCallback(async (userId) => {
    try {
      const { data, error } = await supabase
        ...
        .eq('user_id', userId)
        .single();
      if (error || !data) return; // no remote data yet — use localStorage defaults
      ...
      if (msgs) {
        try { localStorage.setItem('bos_messages', JSON.stringify(msgs)); MESSAGES_STORE._msgs = null; } catch {}
      }
  ...
  // Push all baker data to Supabase (upsert)
  const pushToSupabase = useCallback(async (userId, snapshot) => {
    ...
    } catch (e) { console.warn('[BakerOS] Push failed:', e.message); }
  }, []);

  // Debounce ref for sync
  const syncTimer = useRef(null);

  // Auto-sync whenever key state changes
  const queueSync = useCallback(() => {
    ...
    }, 2000); // 2s debounce — batch rapid changes
  }, [session, brand, bakerInfo, socialLinks, payHandles, products, categories, inventory, photos, appOrders, customers, invoices, reminders, tier, onboarded, pushToSupabase]);

  // Trigger sync on any state change
  useEffect(() => { queueSync(); }, [queueSync]);

  // On login: hydrate from Supabase
  useEffect(() => {
    if (session?.user?.id) {
