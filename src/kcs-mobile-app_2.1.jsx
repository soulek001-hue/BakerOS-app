...
/* ── HOME ──────────────────────────────────── */
function HomePage({setPage,inventory,tier,orders=ORDERS_INIT,customers=[]}){
  const b=useBrand();
...
function OrdersPage({ extraOrders=[], setAppOrders=()=>{}, customers=[], setCustomers=()=>{}, invoices=[], setInvoices=()=>{}, reminders=[], setReminders=()=>{}, tier="" }){
  ...
  // All orders = app-level persisted orders (appOrders passed as extraOrders)
  const allOrders = extraOrders.length > 0
    ? extraOrders
    : [...extraOrders.filter(eo=>!orders.find(o=>o.id===eo.id)), ...orders];
  ...
  const updS = (id, s) => {
    ...
    show(`Status → ${s} ✓`);
    if (s === "completed") {
      const ord = [...extraOrders, ...orders].find(o => o.id === id);
      if (ord) {
...
function AnalyticsPage({ orders=[], customers=[] }){
  ...
  return <div style={{paddingBottom:96}}><PH title="Analytics" sub="Business performance"/><div style={{padding:"0 12px",display:"flex",flexDirection:"column",gap:11}}>
    ...
    <div className="card" style={{padding:"13px"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><span style={{fontSize:14,fontWeight:700,color:"var(--tx)"}}>Weekly Revenue</span><span style={{fontSize:12,color:"var(--mu)"}}>$575</span></div><SparkBar/></div>
...
function InvoicePage({ payHandles, products=[], invoices=[], setInvoices=()=>{}, customers=[], setCustomers=()=>{} }) {
  ...
  const effectiveInvoices = invoices.length > 0 ? invoices : INVOICE_SEED;
  const b = useBrand();
...
/* ── CALENDAR ──────────────────────────────── */
function CalendarPage({ orders=ORDERS_INIT, reminders=[], setReminders=()=>{} }) {
  const b = useBrand();
...
const MESSAGES_INIT = [
  { id:1, name:"Jasmine Carter", phone:"(210)555-0182", email:"jasmine@email.com", subject:"Custom cake question",    body:"Hi! I'd love a 3-tier birthday cake for my daughter's party on May 10th. Can you do a unicorn theme with rainbow layers? Budget is around $150.",         date:"Apr 24, 10:12 AM", read:false, replied:false, archived:false },
  { id:2, name:"Marcus Webb",    phone:"(210)555-0341", email:"marcus@email.com",  subject:"Wedding order inquiry",    body:"We're getting married June 14th and need a 5-tier cake for 120 guests. We'd like alternating vanilla and chocolate layers with cream cheese frosting.", date:"Apr 23, 3:45 PM",  read:false, replied:false, archived:false },
  { id:3, name:"Priya Nair",     phone:"(830)555-0295", email:"priya@email.com",   subject:"Gluten-free options?",     body:"Hello! Do you offer any gluten-free cakes? My son has celiac disease but we still want to celebrate his birthday with something special.",                date:"Apr 22, 9:30 AM",  read:true,  replied:true,  archived:false },
];
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
                <button className="gbtn" onClick={()=>{
                  ...
                  if(!exists){
                    ...
                    localStorage.setItem("bos_customers",JSON.stringify([newC,...stored]));
                    show("Added to CRM ✓");
        ...
        {step===2 && (
          <>
            ...
            <Fld label="Needed by" hint="When do you need this order ready?">
              <input className="field" type="date" value={con.dueDate} onChange={e=>setCon(p=>({...p,dueDate:e.target.value}))}/>
            </Fld>
...
function clearAllData() {
  const keys = [
    ...
    "bos_orders","bos_customers","bos_invoices","bos_reminders","bos_orders","bos_customers","bos_invoices","bos_reminders",
...
export default function App() {
  ...
  const [appOrders,    setAppOrders]   = useLS("bos_orders",    ORDERS_INIT);
  const [customers,    setCustomers]   = useLS("bos_customers",  CUSTS_INIT);
  ...
  const hydrateFromSupabase = useCallback(async (userId) => {
    ...
    try {
      ...
      if (d.bakerInfo)   setBakerInfo(d.bakerInfo);
  ...
  const renderPage = () => {
    switch(page) {
      ...
      case "crm":         return <CRMPage customers={customers} setCustomers={setCustomers}/>;
      ...
      case "orderform":   return <OrderFormPage products={products} photos={photos} addOrder={(o)=>{
        setAppOrders(p=>[o,...p]);
