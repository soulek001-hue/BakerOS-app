...
/* ── INVOICES ──────────────────────────────── */
function InvoicePage({ payHandles, products = [] }) {
  const b = useBrand();
  ...
  return (
    <div style={{paddingBottom:96}}>
      ...
      {/* Invoice list */}
      <div style={{padding:"0 12px",display:"flex",flexDirection:"column",gap:9}}>
        {(()=>{ const filteredInvoices = invFilter==="all" ? invoices : invoices.filter(i=>i.status===invFilter); return filteredInvoices; })().map(inv=>{
...
function CalendarPage() {
  ...
  // Next month holidays for advance planning
      ...
      {nextMonthHolidays.length > 0 && (
        <div style={{padding:"0 12px 12px"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:9}}>
            <div style={{fontSize:13,fontWeight:700,color:"var(--tx)"}}>Coming Up in {nextMonthLabel}</div>
            <div style={{fontSize:10,fontWeight:700,background:"color-mix(in srgb,#C47B00 14%,var(--sf))",color:"#C47B00",borderRadius:20,padding:"2px 8px",flexShrink:0}}>Plan ahead</div>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {nextMonthHolidays.map(h => {
              ...
              const daysAway = Math.round((hDate - today) / (1000*60*60*24));
              return (
                <div key={h.date} onClick={()=>setHolidayPopup(h)} className="card" style={{padding:"11px 13px",display:"flex",alignItems:"center",gap:11,cursor:"pointer",borderLeft:`3px solid ${h.color}`,opacity:0.88}}>
                  <div style={{width:34,height:34,borderRadius:9,background:`color-mix(in srgb,${h.color} 14%,var(--sf))`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>{h.emoji}</div>
                  <div style={{flex:1}}>
                    <div style={{fontSize:13,fontWeight:700,color:"var(--tx)"}}>{h.name}</div>
                    <div style={{fontSize:11,color:"var(--mu)",marginTop:1}}>{hDate.toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric"})} · in {daysAway} days</div>
                  </div>
                  <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4,flexShrink:0}}>
                    <span style={{fontSize:10,fontWeight:700,background:`color-mix(in srgb,${h.color} 14%,var(--sf))`,color:h.color,borderRadius:20,padding:"2px 8px"}}>Start prep</span>
                    <span style={{fontSize:16,color:"var(--mu)"}}>›</span>
                  </div>
                </div>
              );
          ...
          <div className="sheet-panel">
            ...
            <div style={{padding:"4px 16px 0"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
                ...
                <button className="gbtn" onClick={()=>{
                  setMessages(p=>p.map(m=>m.id===sel.id?{...m,archived:!m.archived}:m));
...
export default function App() {
  ...
  const renderPage = () => {
    switch(page) {
      ...
      case "invoices":    return <GateWall feature="invoicing"><InvoicePage payHandles={payHandles} products={products}/></GateWall>;
