  ...
  return (
    <style>{`
      ...
      *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
      html,body{height:100%;overflow:hidden;font-family:var(--fb);background:var(--bg);color:var(--tx);-webkit-font-smoothing:antialiased;}
      ...
      .sheet-panel{position:absolute;bottom:0;left:0;right:0;background:var(--sf);border-radius:20px 20px 0 0;padding:0 0 calc(16px + var(--sb));animation:slideUp .3s cubic-bezier(.22,.68,0,1.1) both;max-height:90vh;overflow-y:auto;}
          ...
          {DMORE.map(item=>{
            ...
            return(
              <button key={item.id} onClick={()=>{setPage(item.id);onClose();}} style={{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"11px 12px",borderRadius:11,border:"none",background:act?`color-mix(in srgb,${b.theme.primary} 12%,var(--sf))`:"transparent",cursor:"pointer",fontFamily:"var(--fb)",marginBottom:2,opacity:locked?.7:1}}>
                ...
                <span style={{fontSize:14,fontWeight:act?700:500,color:act?b.theme.primary:"var(--tx)"}}>{item.id==="crm"?(canAccess(tier,"crm")?"CRM":"Customers"):item.label}</span>
...
function ReceiptPage({onAddToInventory}){
  const[step,setStep]=useState("upload");const[imgURL,setImgURL]=useState(null);const[imgB64,setImgB64]=useState(null);const[cat,setCat]=useState(null);const[parsed,setParsed]=useState(null);const[items,setItems]=useState([]);const[sel,setSel]=useState(new Set());const[added,setAdded]=useState([]);const[t,show]=useToast();
  const fR=useRef();const cR=useRef();
  const load=useCallback(file=>{if(!file||!file.type.startsWith("image/"))return;setImgURL(URL.createObjectURL(file));const r=new FileReader();r.onload=e=>{setImgB64(e.target.result.split(",")[1]);setStep("category");};r.readAsDataURL(file);},[]);
  const scan=async()=>{setStep("scanning");try{const result=await parseReceipt(imgB64,cat);setParsed(result);const pi=result.items.map((item,i)=>({...item,id:i,icon:getIcon(item.name),category:cat,supplier:result.store||"Unknown",min:0,reorderQty:Math.ceil(item.qty*2)}));setItems(pi);setSel(new Set(pi.map(i=>i.id)));setStep("review");}catch{show("Could not read - try a clearer photo","error");setStep("category");}};
  const doImport=()=>{const toAdd=items.filter(i=>sel.has(i.id)&&i.name.trim());if(!toAdd.length){show("Select at least one item","error");return;}const fmt=toAdd.map((i,idx)=>({id:Date.now()+idx,name:i.name,cat:i.category,unit:i.unit,qty:i.qty,min:i.min,cost:i.unitCost,supplier:i.supplier,reorderQty:i.reorderQty,icon:i.icon}));onAddToInventory(fmt);setAdded(fmt);setStep("done");};
  const reset=()=>{setStep("upload");setImgURL(null);setImgB64(null);setCat(null);setParsed(null);setItems([]);setSel(new Set());setAdded([]);};
  return(
    <div style={{paddingBottom:96}}>
      <Toast t={t}/>
      <PH title="Scan Receipt" sub="Auto-import to inventory"/>
      <div style={{padding:"0 12px",display:"flex",flexDirection:"column",gap:11}}>
        {step==="upload"&&<>
          <div onClick={()=>fR.current.click()} style={{border:"2px dashed var(--bd)",borderRadius:18,padding:"38px 20px",textAlign:"center",cursor:"pointer",background:"var(--sf)"}}>
            <div style={{fontSize:42,marginBottom:10,animation:"pulse 2.5s ease-in-out infinite"}}>📷</div>
            <div style={{fontFamily:"var(--fd)",fontSize:18,fontWeight:700,color:"var(--tx)",marginBottom:5}}>Upload a receipt</div>
            <p style={{color:"var(--mu)",fontSize:13,lineHeight:1.5,marginBottom:14}}>We'll read every item automatically using AI</p>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,maxWidth:240,margin:"0 auto"}}>
              <button className="pbtn" style={{fontSize:13}}>🖼 Gallery</button>
              <button className="gbtn" onClick={e=>{e.stopPropagation();cR.current.click();}} style={{fontSize:13,borderRadius:12,padding:"12px",color:"var(--tx)"}}>📱 Camera</button>
            </div>
            <input ref={fR} type="file" accept="image/*" style={{display:"none"}} onChange={e=>load(e.target.files[0])}/>
            <input ref={cR} type="file" accept="image/*" capture="environment" style={{display:"none"}} onChange={e=>load(e.target.files[0])}/>
          </div>
          <div className="card" style={{padding:"13px 14px"}}><div style={{fontSize:12,fontWeight:700,color:"var(--tx)",marginBottom:8}}>📌 Tips for best results</div>{[["💡","Good lighting - no shadows"],["📐","Lay flat on a surface"],["🔍","All items must be in frame"],["🚫","Avoid glare on shiny paper"]].map(([ic,tip])=><div key={tip} style={{display:"flex",alignItems:"center",gap:9,marginBottom:6,fontSize:12,color:"var(--mu)"}}><span style={{fontSize:14}}>{ic}</span>{tip}</div>)}</div>
        </>}
        {step==="category"&&<>
          <div style={{borderRadius:14,overflow:"hidden",position:"relative"}}><img src={imgURL} alt="" style={{width:"100%",height:160,objectFit:"cover",display:"block"}}/><div style={{position:"absolute",inset:0,background:"rgba(0,0,0,.28)",display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{background:"rgba(0,0,0,.55)",borderRadius:9,padding:"6px 12px",color:"#fff",fontSize:12,fontWeight:700}}>📷 Receipt Loaded</div></div></div>
          <div style={{fontFamily:"var(--fd)",fontSize:17,fontWeight:700,color:"var(--tx)"}}>What is this receipt for?</div>
          {RCATS.map(rc=><button key={rc.id} onClick={()=>setCat(rc.id)} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 13px",borderRadius:13,border:`1.5px solid ${cat===rc.id?"var(--p)":"var(--bd)"}`,background:cat===rc.id?`color-mix(in srgb,var(--p) 8%,var(--sf))`:"var(--sf)",cursor:"pointer",fontFamily:"var(--fb)"}}><span style={{fontSize:22}}>{rc.icon}</span><span style={{fontSize:14,fontWeight:700,color:"var(--tx)"}}>{rc.label}</span>{cat===rc.id&&<span style={{marginLeft:"auto",color:"var(--p)",fontWeight:800,fontSize:16}}>✓</span>}</button>)}
          <button className="pbtn" onClick={scan} disabled={!cat} style={{width:"100%"}}>🔍 Scan Receipt →</button>
          <button className="gbtn" onClick={reset} style={{width:"100%",padding:"12px"}}>← Back</button>
        </>}
        {step==="scanning"&&<div style={{textAlign:"center",padding:"44px 20px"}}><div style={{width:56,height:56,borderRadius:"50%",background:"var(--p)",margin:"0 auto 16px",display:"flex",alignItems:"center",justifyContent:"center",animation:"pulse 1.2s ease-in-out infinite"}}><div style={{width:20,height:20,border:"3px solid rgba(255,255,255,.4)",borderTop:"3px solid #fff",borderRadius:"50%",animation:"spin .8s linear infinite"}}/></div><div style={{fontFamily:"var(--fd)",fontSize:19,fontWeight:700,color:"var(--tx)",marginBottom:5}}>Reading receipt...</div><p style={{color:"var(--mu)",fontSize:13}}>AI is extracting items and prices.</p></div>}
        {step==="review"&&parsed&&<>
          <div className="card" style={{padding:"11px 13px",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{display:"flex",alignItems:"center",gap:9}}><img src={imgURL} alt="" style={{width:34,height:34,borderRadius:7,objectFit:"cover"}}/><div><div style={{fontSize:12,fontWeight:700,color:"var(--tx)"}}>{parsed.store||"Receipt"}</div><div style={{fontSize:10,color:"var(--mu)"}}>{RCATS.find(c=>c.id===cat)?.label}</div></div></div><div style={{textAlign:"right"}}><div style={{fontSize:10,color:"var(--mu)"}}>Total</div><div style={{fontSize:14,fontWeight:800,color:"var(--tx)"}}>${parsed.total?.toFixed(2)||"-"}</div></div></div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{fontSize:13,fontWeight:700,color:"var(--tx)"}}>{items.length} items detected</div><button onClick={()=>sel.size===items.length?setSel(new Set()):setSel(new Set(items.map(i=>i.id)))} style={{fontSize:12,color:"var(--p)",fontWeight:600,background:"none",border:"none",cursor:"pointer"}}>{sel.size===items.length?"Deselect all":"Select all"}</button></div>
          {items.map(item=><div key={item.id} onClick={()=>setSel(p=>{const n=new Set(p);n.has(item.id)?n.delete(item.id):n.add(item.id);return n;})} className="card row" style={{padding:"11px 13px",cursor:"pointer",border:`1.5px solid ${sel.has(item.id)?"var(--p)":"var(--bd)"}`}}><div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:16,height:16,borderRadius:4,border:`2px solid ${sel.has(item.id)?"var(--p)":"var(--bd)"}`,background:sel.has(item.id)?"var(--p)":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{sel.has(item.id)&&<span style={{color:"#fff",fontSize:10,fontWeight:800}}>✓</span>}</div><span style={{fontSize:20}}>{item.icon}</span><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:700,color:"var(--tx)"}}>{item.name}</div><div style={{fontSize:11,color:"var(--mu)",marginTop:1}}>{item.qty} {item.unit} · ${item.unitCost}/{item.unit}</div></div><div style={{fontWeight:800,fontSize:13,color:"var(--p)",flexShrink:0}}>${(item.totalCost||0).toFixed(2)}</div></div></div>)}
          <button className="pbtn" onClick={doImport} disabled={sel.size===0} style={{width:"100%"}}>📦 Add {sel.size} Item{sel.size!==1?"s":""} to Inventory</button>
          <button className="gbtn" onClick={reset} style={{width:"100%",padding:"12px"}}>🔄 New Receipt</button>
        </>}
        {step==="done"&&<>
          <div style={{textAlign:"center",padding:"36px 20px",animation:"pop .5s cubic-bezier(.22,.68,0,1.4) both"}}><div style={{fontSize:52,marginBottom:12}}>🎉</div><div style={{fontFamily:"var(--fd)",fontSize:21,fontWeight:700,color:"var(--tx)",marginBottom:7}}>{added.length} items added!</div><p style={{color:"var(--mu)",fontSize:13,lineHeight:1.5,marginBottom:20}}>Items are now in your inventory.</p><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><button className="pbtn" onClick={reset}>📷 Scan More</button><button className="gbtn" style={{padding:"13px",borderRadius:12,color:"var(--tx)"}}>📦 Inventory</button></div></div>
          <div className="card" style={{overflow:"hidden"}}>{added.map((item,i)=><div key={item.id} style={{padding:"10px 13px",borderBottom:i<added.length-1?"1px solid var(--bd)":"none",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{display:"flex",alignItems:"center",gap:9}}><span style={{fontSize:20}}>{item.icon}</span><div><div style={{fontSize:12,fontWeight:700,color:"var(--tx)"}}>{item.name}</div><div style={{fontSize:10,color:"var(--mu)"}}>{item.qty} {item.unit}</div></div></div><div style={{fontSize:12,fontWeight:700,color:"var(--p)"}}>${(item.cost*item.qty).toFixed(2)}</div></div>)}</div>
        </>}
      </div>
    </div>
  );
...
function CustomerStorefrontPage({ products, categories, photos, brand, socialLinks, bakerInfo, setPage }) {
  ...
  return (
    <div style={{paddingBottom:32,background:"#FDF6EC",minHeight:"100vh"}}>
      <Toast t={t}/>
...
function OnboardingFlow({ step, setStep, bakery, setBakery, products, setProducts, errors, setErrors, linkCopied, setLinkCopied, brand, tier, onComplete }) {
  ...
  return (
    <div style={{maxWidth:430,margin:"0 auto",height:"100vh",background:"var(--bg)",display:"flex",flexDirection:"column",overflow:"hidden",boxShadow:"0 0 60px rgba(0,0,0,.15)"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"0 0 100px",WebkitOverflowScrolling:"touch"}}>
        ...
        {/* STEP 1: WELCOME */}
        {step===1 && (
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"0 24px",textAlign:"center",minHeight:"90vh",justifyContent:"center"}}>
      ...
      {step >= 2 && step <= 6 && (
        <div style={{padding:"12px 20px",borderTop:"1px solid var(--bd)",background:"var(--sf)",flexShrink:0}}>
...
export default function App() {
  ...
  const renderPage = () => {
    switch(page) {
      ...
      case "payments":    return <PaymentsPage payHandles={payHandles} setPayHandles={setPayHandles}/>;      case "receipt":     return <GateWall feature="receipt_scan"><ReceiptPage onAddToInventory={addToInventory}/></GateWall>;
  ...
  return (
    <TierCtx.Provider value={tierCtxValue}>
      <Ctx.Provider value={brand}>
        <GlobalCSS brand={brand}/>
        <div style={{maxWidth:430,margin:"0 auto",height:"100vh",position:"relative",background:"var(--bg)",overflow:"hidden",display:"flex",flexDirection:"column",boxShadow:"0 0 60px rgba(0,0,0,.15)"}}>
          ...
          <div className="page-scroll" style={{flex:1,paddingTop:44,paddingBottom:inBottom?72:16}}>
            {renderPage()}
