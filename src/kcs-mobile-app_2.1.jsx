...
function NFCPage(){
    ...
    <div style={{paddingBottom:96}}>
      ...
      <div style={{padding:"0 12px",display:"flex",flexDirection:"column",gap:12}}>
        ...
        <div className="card" style={{padding:"28px 20px",textAlign:"center"}}>
          <div className="qr-nfc-wrap" style={{border:`3px solid ${b.theme.primary}`,borderRadius:16,padding:4,display:"inline-block",margin:"0 auto 16px",background:"#fff"}}><QRCodeDisplay url={`https://bakeros.app/store/${bakerInfo?.username||"bakeros"}`} size={132} primaryColor={b.theme.primary}/></div>
          <div style={{fontFamily:"var(--fd)",fontSize:17,fontWeight:700,color:"var(--tx)",marginBottom:4}}>BakerOS Storefront</div>
          <div style={{fontSize:12,color:"var(--mu)",marginBottom:16}}>{`bakeros.app/store/${bakerInfo?.username||"bakeros"}`}</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
  ...
  if(canvas){const a=document.createElement("a");a.download="bakeros-qr.png";a.href=canvas.toDataURL("image/png");a.click();show("QR downloaded ✓");}
            ...
            <button className="gbtn" onClick={()=>{if(navigator.clipboard)navigator.clipboard.writeText(`https://bakeros.app/store/${bakerInfo?.username||"bakeros"}`);show("Link copied ✓");}} style={{padding:"12px",borderRadius:12,color:"var(--tx)",fontSize:13}}>Copy Link</button>
          </div>
        </div>
        {/* Upgrade to Pro CTA */}
...
function OnboardingFlow({ step, setStep, bakery, setBakery, products, setProducts, errors, setErrors, linkCopied, setLinkCopied, brand, tier, onComplete }) {
  ...
  if(canvas){const a=document.createElement("a");a.download="bakeros-qr.png";a.href=canvas.toDataURL("image/png");a.click();}
  else {if(navigator.clipboard)navigator.clipboard.writeText(`https://bakeros.app/store/${bakery.username||"bakeros"}`); }
}} style={{width:"100%",padding:"10px",borderRadius:11,border:`1.5px solid ${primary}`,background:"transparent",color:primary,fontFamily:"var(--fb)",fontWeight:700,fontSize:13,cursor:"pointer"}}>Download QR Code</button>
            </div>
