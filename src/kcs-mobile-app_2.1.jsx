...
function CustomerStorefrontPage({ products, categories, photos, brand, socialLinks, bakerInfo, setPage, isOwnerPreview = false }) {
  ...
  if(navigator.share){navigator.share({title:brand?.storeName||bakerInfo?.name||"My Bakery",text:"Check out my bakery!",url}).catch(()=>{});}
      ...
      {/* Category filter tabs — only show on baker's own preview, not on public storefront */}
      {isOwnerPreview && usedCats.length > 0 && (
      ...
      {/* Products — only show on baker's own preview, not on public storefront */}
      {isOwnerPreview && <div style={{padding:"16px 12px 0"}}>
        {filterCat === "all" ? (
          <>
            {usedCats.map(cat => {
...
function OrderFormPage({ products=[], photos=[], addOrder }){
  ...
  const validate = () => {
    ...
    if (step===2 && !con.dueDate)  e.dueDate = "Due date required";
        ...
        {step===1 && (
          <>
            ...
            <Fld label="Needed by" hint="When do you need this order ready?">
              ...
              {errs.dueDate&&<div style={{color:"#DC2626",fontSize:11,marginTop:3}}>{errs.dueDate}</div>}
            </Fld>
