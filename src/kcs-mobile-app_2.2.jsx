...
// ── API fetch helper — sends Supabase session token for server-side auth ──────
// Never expose a shared secret in frontend code. APIs validate via Supabase JWT.
...
const FEATURES = {
  ...
  inventory_deduction:     ["pro","elite"],         // recipe → inventory auto-deduct
  holiday_marketing:       ["growth","pro","elite"], // holiday ideas + marketing reminders
  ...
  marketing:               ["pro","elite"],            // full campaign builder
...
function SubscriptionPage() {
  ...
  const plans = [
    {
      ...
      features:[
        "Public storefront with order form",
        "Up to 15 products",
        "Basic order tracking & calendar",
        "Customer list (up to 25 customers)",
        "Recipe book with costing calculator",
        "Photo gallery",
        "QR code for storefront",
        "Free NFC keychain (after address confirmation)",
    ...
    },
    {
      ...
      features:[
        ...
        "Custom storefront branding & logo",
        "Remove BakerOS branding",
        "Customer messaging inbox",
        "Automated order confirmation emails",
        "Invoicing with Cash App, Venmo & Zelle",
        "Inventory management",
        "Holiday marketing calendar & ideas",
        "Payment handle integrations",
      ],
    },
    {
      ...
      features:[
        ...
        "Recipe → inventory auto-deduction",
        "Recipe profit margin calculator",
        "SMS & email marketing campaigns",
        "AI Campaign Builder",
        "Saved reply templates",
        "NFC lead capture → auto-saved to CRM",
        "Advanced CRM (tags, notes, birthdays, allergies)",
    ...
    },
    {
      ...
      features:[
        "Everything in Pro",
        "AI receipt scanner (auto-imports to inventory)",
        "Tax export tools",
  ...
  const COMPARISON_SECTIONS = [
    {
      title: "Core — All Plans",
      rows: [
        ["Public Storefront + Order Form",     true,   true,   true,   true  ],
        ["QR Code + Free NFC Keychain",        true,   true,   true,   true  ],
        ["Order Tracking & Calendar",          true,   true,   true,   true  ],
        ["Products",                           "15",   "∞",    "∞",    "∞"   ],
        ["Customer List",                      "25",   "∞",    "∞",    "∞"   ],
        ["Recipe Book + Cost Calculator",      true,   true,   true,   true  ],
    ...
    },
    {
      ...
      rows: [
        ...
        ["Order Confirmation Emails",          false,  true,   true,   true  ],
        ["Invoicing (Cash App/Venmo/Zelle)",   false,  true,   true,   true  ],
        ["Inventory Management",               false,  true,   true,   true  ],
        ["Holiday Marketing Calendar",         false,  true,   true,   true  ],
        ["Payment Handle Integrations",        false,  true,   true,   true  ],
      ]
    },
    {
      title: "Pro — $39.99/mo",
      ...
      rows: [
        ...
        ["Recipe Profit Margin Calculator",    false,  false,  true,   true  ],
        ["SMS & Email Marketing",              false,  false,  true,   true  ],
        ["AI Campaign Builder",                false,  false,  true,   true  ],
        ["Saved Reply Templates",              false,  false,  true,   true  ],
        ["NFC Lead Capture",                   false,  false,  true,   true  ],
        ["Advanced CRM",                       false,  false,  true,   true  ],
        ["Analytics Dashboard",                false,  false,  true,   true  ],
        ["Customer Segmentation",              false,  false,  true,   true  ],
        ["Automated Follow-Ups",               false,  false, "Soon", "Soon" ],
        ["Discount Codes",                     false,  false, "Soon", "Soon" ],
      ]
    },
    {
      ...
      rows: [
        ...
        ["Multi-User Accounts",                false,  false,  false, "Soon" ],
        ["Priority Support",                   false,  false,  false,  true  ],
      ]
    },
  ];

  const tierColors = ["#6B7280","#3D5A99","#C47B00","#7C5CBF"];
  ...
  return (
    <style>{`
      ...
      *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
      html,body,#root{margin:0;padding:0;width:100%;height:100%;overscroll-behavior:none;}
      ...
      .sheet-panel{position:absolute;bottom:0;left:0;right:0;background:var(--sf);border-radius:20px 20px 0 0;padding:0;animation:slideUp .3s cubic-bezier(.22,.68,0,1.1) both;max-height:92dvh;overflow-y:auto;-webkit-overflow-scrolling:touch;}
...
const DMORE = [
  ...
  {id:"advanced_crm", icon:"🧠", label:"Advanced CRM"},
...
function Drawer({open,onClose,setPage,page,tier,nfcNewCount=0}){
  ...
  const itemFeature={
    ...
    marketing:    "marketing",            // Pro+ (full campaigns — Growth sees upgrade funnel)
    inventory:    "inventory",            // Growth+
    advanced_crm: "crm_advanced",         // Pro+
    analytics:    "analytics",            // Pro+
    nfc:          "nfc_lead_capture",     // Pro+
    receipt:      "receipt_scan",         // Elite
    accounting:   "accounting_export",    // Elite
    // customers: no lock — available to all tiers
  };
  return(
    ...
    <div style={{paddingBottom:96}}>
      ...
      <div style={{padding:"0 12px",marginTop:-14,position:"relative",zIndex:2}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          ...
          {/* Customers */}
          <div className="card" style={{padding:"13px 14px",cursor:"pointer"}} onClick={()=>setPage("customers")}>
            <div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontSize:20}}>👥</span></div>
        ...
        const TIER_BUTTONS = {
          starter: {
            ...
            buttons:[
              ...
              {icon:"👥", label:"Customers",    sub:"View & add customers",   page:"customers",  color:"#3D5A99"},
          ...
          growth: {
            ...
            buttons:[
              ...
              {icon:"👥", label:"Customers",     sub:"Customer list & history",     page:"customers", color:"#3D5A99"},
...
function OrdersPage({ extraOrders=[], setAppOrders=()=>{}, customers=[], setCustomers=()=>{}, invoices=[], setInvoices=()=>{}, reminders=[], setReminders=()=>{}, tier="" }){
  ...
  const deleteOrder = async (id) => {
    ...
    setAppOrders(p => p.filter(o => o.id !== id));
    setSel(null);
    setShowDeleteOrder(false);
    show("Order deleted");
    const { data: { session } } = await supabase.auth.getSession();
    if (session?.user?.id) {
      supabase.from("baker_orders").delete().eq("id", id).eq("baker_id", session.user.id);
    }
  };

  useEffect(() => { setShowDeleteOrder(false); }, [sel?.id]);
  ...
  const openSMS = () => {
    ...
    // Blur any focused element to prevent iOS auto-scroll to focused input
    if (document.activeElement) document.activeElement.blur();
  };

  useEffect(() => {
    if (!showSMSPicker) return;
    [50, 150, 300].forEach(delay => {
      setTimeout(() => {
        const el = document.getElementById("sms-picker-sheet");
  ...
  useEffect(() => {
    if (!showManageSMS) return;
    [50, 150, 300].forEach(delay => {
      setTimeout(() => {
        const el = document.getElementById("manage-sms-sheet");
        if (el) el.scrollTop = 0;
      }, delay);
    });
  }, [showManageSMS]);
    ...
    <div style={{paddingBottom:96}}>
      ...
      {sel && !showSMSPicker && !showCancelMsg && (
        <div className="sheet">
          ...
          <div className="sheet-panel" style={{maxHeight:"90dvh",overflowY:"auto"}}>
            <Handle/>
            <div style={{padding:"6px 16px calc(96px + env(safe-area-inset-bottom, 16px))"}}>
              ...
              {!showDeleteOrder ? (
                <button
                  ...
                  style={{width:"100%",marginTop:10,padding:"11px",borderRadius:11,border:"1.5px solid #FCA5A5",background:"transparent",color:"#DC2626",fontFamily:"var(--fb)",fontWeight:700,fontSize:13,cursor:"pointer"}}
                >
                  🗑 Delete Order
                </button>
              ) : (
                <div style={{marginTop:10,background:"#FEF2F2",borderRadius:12,padding:"13px 14px"}}>
                  <div style={{fontSize:13,fontWeight:700,color:"#DC2626",marginBottom:10,textAlign:"center"}}>Delete this order?</div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
                    <button onClick={()=>setShowDeleteOrder(false)} style={{padding:"10px",borderRadius:10,border:"1px solid var(--bd)",background:"var(--sf)",color:"var(--mu)",fontFamily:"var(--fb)",fontWeight:600,fontSize:13,cursor:"pointer"}}>Cancel</button>
                    <button onClick={()=>deleteOrder(sel.id)} style={{padding:"10px",borderRadius:10,border:"none",background:"#DC2626",color:"#fff",fontFamily:"var(--fb)",fontWeight:700,fontSize:13,cursor:"pointer"}}>Yes, Delete</button>
                  </div>
                </div>
              )}
            </div>
      ...
      {showSMSPicker && sel && (
        <div className="sheet">
          ...
          <div className="sheet-panel" style={{paddingBottom:"env(safe-area-inset-bottom, 16px)"}}>
...
function RecipePage({ inventory, setInventory, products, recipes=[], setRecipes=()=>{} }) {
  ...
  const hasDeduction = canAccess(recTier, "inventory_deduction"); // Pro+ (for auto-deducting)
  const [t, show] = useToast();
  ...
  const blankRecipe = {
    ...
    sellingPrice:"",     // baker's actual selling price per unit
    laborMins:0,         // minutes to make one batch
    laborRate:15,        // $ per hour for labor
    packagingCost:0,     // $ packaging cost per batch
    markupTarget:3,      // target markup multiplier (used for suggested price)
  };
  ...
  const calcCost = (recipe) => {
    ...
    // Ingredient cost: cost-per-unit × qty used per batch
    let ingredientCost = 0;
    let missingCost = false;
    recipe.ingredients.forEach(ing => {
      const item = inventory.find(i => i.id === ing.inventoryId);
      if (!item) return;
      if (!item.cost || item.cost === 0) { missingCost = true; return; }
      ingredientCost += (parseFloat(ing.qty) || 0) * (parseFloat(item.cost) || 0);
    });

    const laborCost   = ((parseFloat(recipe.laborMins) || 0) / 60) * (parseFloat(recipe.laborRate) || 15);
    const packCost    = parseFloat(recipe.packagingCost) || 0;
    const totalBatch  = ingredientCost + laborCost + packCost;
    const batchYield  = parseFloat(recipe.batchYield) || 1;
    const costPerUnit = totalBatch / batchYield;
    const sellingPrice = parseFloat(recipe.sellingPrice) || 0;
    const markup      = parseFloat(recipe.markupTarget) || 3;
    const suggestedPrice = costPerUnit * markup;
    const profitPerUnit  = sellingPrice > 0 ? sellingPrice - costPerUnit : null;
    const margin         = sellingPrice > 0 ? ((sellingPrice - costPerUnit) / sellingPrice) * 100 : null;

    return {
      ingredientCost: parseFloat(ingredientCost.toFixed(2)),
      laborCost:      parseFloat(laborCost.toFixed(2)),
      packCost:       parseFloat(packCost.toFixed(2)),
      totalBatch:     parseFloat(totalBatch.toFixed(2)),
      costPerUnit:    parseFloat(costPerUnit.toFixed(2)),
      suggestedPrice: parseFloat(suggestedPrice.toFixed(2)),
      profitPerUnit:  profitPerUnit !== null ? parseFloat(profitPerUnit.toFixed(2)) : null,
    ...
    <div style={{paddingBottom:96}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  {cost && (
                    <div style={{display:"flex",gap:8,marginTop:4,flexWrap:"wrap"}}>
                      <span style={{fontSize:10,fontWeight:700,color:"var(--mu)"}}>Cost: <span style={{color:"var(--tx)"}}>${cost.costPerUnit}/{recipe.yieldUnit||"unit"}</span></span>
                      {cost.margin !== null && (
                        <span style={{fontSize:10,fontWeight:700,color:cost.margin>=50?"#16A34A":cost.margin>=30?"#C47B00":"#DC2626"}}>
                          {cost.margin.toFixed(0)}% margin
                        </span>
                      )}
                    </div>
                  )}
      ...
      {sel && !showForm && (
        <div className="sheet">
          ...
          <div className="sheet-panel" style={{maxHeight:"92dvh",overflowY:"auto",WebkitOverflowScrolling:"touch",touchAction:"pan-y",paddingBottom:0}}>
            <div style={{width:36,height:4,background:"var(--bd)",borderRadius:2,margin:"12px auto 6px"}}/>
            <div style={{padding:"4px 16px calc(110px + env(safe-area-inset-bottom,0px))"}}>
              ...
              {(()=>{
                ...
                if (!cost) return null;
                const marginColor = cost.margin === null ? "var(--mu)" : cost.margin >= 50 ? "#16A34A" : cost.margin >= 30 ? "#C47B00" : "#DC2626";
                const marginLabel = cost.margin === null ? "Set selling price" : cost.margin >= 60 ? "Excellent margin" : cost.margin >= 40 ? "Good margin" : cost.margin >= 20 ? "Tight margin" : "Below cost risk";
                return (
                  <div style={{background:"var(--bg)",borderRadius:14,padding:"14px 15px",marginBottom:14,border:"1px solid var(--bd)"}}>
                    <div style={{fontSize:11,fontWeight:800,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginBottom:12}}>💰 Recipe Costing</div>

                    {/* Cost breakdown rows */}
                    <div style={{display:"flex",flexDirection:"column",gap:7,marginBottom:12}}>
                      <div style={{display:"flex",justifyContent:"space-between"}}>
                        <span style={{fontSize:12,color:"var(--mu)"}}>Ingredients</span>
                        <span style={{fontSize:13,fontWeight:700,color:"var(--tx)"}}>${cost.ingredientCost.toFixed(2)}</span>
                      </div>
                      {cost.laborCost > 0 && (
                        <div style={{display:"flex",justifyContent:"space-between"}}>
                          <span style={{fontSize:12,color:"var(--mu)"}}>Labor ({sel.laborMins} min @ ${sel.laborRate}/hr)</span>
                          <span style={{fontSize:13,fontWeight:700,color:"var(--tx)"}}>${cost.laborCost.toFixed(2)}</span>
                        </div>
                      )}
                      {cost.packCost > 0 && (
                        <div style={{display:"flex",justifyContent:"space-between"}}>
                          <span style={{fontSize:12,color:"var(--mu)"}}>Packaging</span>
                          <span style={{fontSize:13,fontWeight:700,color:"var(--tx)"}}>${cost.packCost.toFixed(2)}</span>
                        </div>
                      )}
                      <div style={{borderTop:"1px solid var(--bd)",paddingTop:7,display:"flex",justifyContent:"space-between"}}>
                        <span style={{fontSize:12,fontWeight:700,color:"var(--tx)"}}>Total batch cost</span>
                        <span style={{fontSize:13,fontWeight:800,color:"var(--tx)"}}>${cost.totalBatch.toFixed(2)}</span>
                      </div>
                      <div style={{display:"flex",justifyContent:"space-between"}}>
                        <span style={{fontSize:12,fontWeight:700,color:"var(--tx)"}}>Cost per {sel.yieldUnit||"unit"}</span>
                        <span style={{fontSize:14,fontWeight:800,color:"var(--tx)"}}>${cost.costPerUnit.toFixed(2)}</span>
                      </div>
                    </div>

                    {/* Suggested vs actual price */}
                    <div style={{background:`color-mix(in srgb,${marginColor} 8%,var(--sf))`,borderRadius:11,padding:"11px 13px",marginBottom:cost.missingCost?10:0}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                        <span style={{fontSize:12,color:"var(--mu)"}}>Suggested price ({sel.markupTarget||3}× markup)</span>
                        <span style={{fontSize:14,fontWeight:800,color:b.theme.primary}}>${cost.suggestedPrice.toFixed(2)}</span>
                      </div>
                      {sel.sellingPrice && parseFloat(sel.sellingPrice) > 0 ? (
                        <>
                          <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                            <span style={{fontSize:12,color:"var(--mu)"}}>Your selling price</span>
                            <span style={{fontSize:14,fontWeight:800,color:"var(--tx)"}}>${parseFloat(sel.sellingPrice).toFixed(2)}</span>
                          </div>
                          <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                            <span style={{fontSize:12,color:"var(--mu)"}}>Profit per {sel.yieldUnit||"unit"}</span>
                            <span style={{fontSize:14,fontWeight:800,color:marginColor}}>${cost.profitPerUnit?.toFixed(2)||"—"}</span>
                          </div>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                            <span style={{fontSize:12,color:"var(--mu)"}}>Gross margin</span>
                            <div style={{textAlign:"right"}}>
                              <span style={{fontSize:16,fontWeight:900,color:marginColor}}>{cost.margin?.toFixed(0)||"—"}%</span>
                              <div style={{fontSize:9,fontWeight:700,color:marginColor,marginTop:1}}>{marginLabel}</div>
                            </div>
                          </div>
                        </>
                      ) : (
                        <div style={{fontSize:11,color:"var(--mu)",fontStyle:"italic"}}>Add your selling price in Edit to see profit margin</div>
                      )}
                    </div>

                    {cost.missingCost && (
                      <div style={{fontSize:11,color:"#C47B00",marginTop:8,lineHeight:1.5}}>
                        ⚠ Some ingredients have no cost set — add costs in Inventory for accurate calculations
                      </div>
                    )}
                  </div>
                );
              ...
              {hasDeduction ? (
                ...
                <div style={{background:`color-mix(in srgb,#7C5CBF 6%,var(--sf))`,border:"1.5px solid #7C5CBF30",borderRadius:14,padding:"14px",marginBottom:14,textAlign:"center"}}>
                  ...
                  <div style={{fontSize:12,color:"var(--mu)",lineHeight:1.6,marginBottom:12}}>
                    Upgrade to Pro to automatically deduct ingredients from your inventory when orders are completed — with smart unit conversion built in.
                  </div>
      ...
      {showForm && (
        <div className="sheet">
          ...
          <div className="sheet-panel" style={{maxHeight:"92vh",overflowY:"auto",WebkitOverflowScrolling:"touch",touchAction:"pan-y",paddingBottom:0}}>
            <div style={{width:36,height:4,background:"var(--bd)",borderRadius:2,margin:"12px auto 6px"}}/>
            ...
            <div style={{padding:"0 16px calc(100px + env(safe-area-inset-bottom,0px))",display:"flex",flexDirection:"column",gap:13}}>
              ...
              <div style={{borderTop:"1px solid var(--bd)",paddingTop:13}}>
                <div style={{fontSize:11,fontWeight:800,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginBottom:11}}>💰 Costing & Pricing</div>

                <Fld label="Your selling price per unit ($)" hint={`e.g. what you charge per ${form.yieldUnit||"unit"}`}>
                  <input className="field" type="number" min="0" step="0.01" placeholder="0.00" value={form.sellingPrice} onChange={e=>setForm(p=>({...p,sellingPrice:e.target.value}))}/>
                </Fld>

                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9,marginTop:11}}>
                  <Fld label="Labor time (mins/batch)" hint="Time to make one full batch">
                    <input className="field" type="number" min="0" step="1" placeholder="0" value={form.laborMins||""} onChange={e=>setForm(p=>({...p,laborMins:parseFloat(e.target.value)||0}))}/>
                  </Fld>
                  <Fld label="Labor rate ($/hr)" hint="Your hourly rate">
                    <input className="field" type="number" min="0" step="1" placeholder="15" value={form.laborRate||""} onChange={e=>setForm(p=>({...p,laborRate:parseFloat(e.target.value)||15}))}/>
                  </Fld>
                </div>

                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9,marginTop:11}}>
                  <Fld label="Packaging cost ($/batch)">
                    <input className="field" type="number" min="0" step="0.01" placeholder="0.00" value={form.packagingCost||""} onChange={e=>setForm(p=>({...p,packagingCost:parseFloat(e.target.value)||0}))}/>
                  </Fld>
                  <Fld label="Target markup" hint="Suggested price = cost × this">
                    <select className="field" value={form.markupTarget||3} onChange={e=>setForm(p=>({...p,markupTarget:parseFloat(e.target.value)}))}>
                      {[["2×","2"],["2.5×","2.5"],["3×","3"],["3.5×","3.5"],["4×","4"],["5×","5"]].map(([lbl,val])=>(
                ...
                {(()=>{
                  const preview = calcCost(form);
                  if (!preview || form.ingredients.length === 0) return null;
                  return (
                    <div style={{background:`color-mix(in srgb,${b.theme.primary} 6%,var(--sf))`,borderRadius:11,padding:"11px 13px",marginTop:11,border:`1px solid ${b.theme.primary}20`}}>
                      <div style={{fontSize:10,fontWeight:800,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginBottom:8}}>Live Cost Preview</div>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                        <span style={{fontSize:12,color:"var(--mu)"}}>Batch cost</span>
                        <span style={{fontSize:13,fontWeight:700,color:"var(--tx)"}}>${preview.totalBatch.toFixed(2)}</span>
                      </div>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                        <span style={{fontSize:12,color:"var(--mu)"}}>Cost per {form.yieldUnit||"unit"}</span>
                        <span style={{fontSize:13,fontWeight:700,color:"var(--tx)"}}>${preview.costPerUnit.toFixed(2)}</span>
                      </div>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                        <span style={{fontSize:12,color:"var(--mu)"}}>Suggested price ({form.markupTarget||3}×)</span>
                        <span style={{fontSize:13,fontWeight:700,color:b.theme.primary}}>${preview.suggestedPrice.toFixed(2)}</span>
                      </div>
                      {preview.margin !== null && (
                        <div style={{display:"flex",justifyContent:"space-between"}}>
                          <span style={{fontSize:12,color:"var(--mu)"}}>Gross margin</span>
                          <span style={{fontSize:14,fontWeight:900,color:preview.margin>=50?"#16A34A":preview.margin>=30?"#C47B00":"#DC2626"}}>{preview.margin.toFixed(0)}%</span>
                        </div>
                      )}
                    </div>
                  );
...
function MarketingPage({ setPage, bakerInfo = {}, products = [], customers = [], tier = "starter" }){
  ...
  const canMarketing = canAccess(mktTier, "marketing"); // Pro+ full access
  const canSend = canAccess(mktTier, "marketing_sms");   // Pro+ can send campaigns
  const isPro   = canMarketing;
  const isGrowth = canAccess(mktTier, "order_dashboard") && !isPro; // Growth sees upgrade funnel
...
Return ONLY valid JSON (no markdown, no explanation):
{
  ...
  // ── AI Builder view ────────────────────────────────────────────────────────
  // Gate AI campaigns — must be Pro or Elite
  // Growth bakers see an upgrade funnel — they can browse but not send
  if (!canMarketing) return (
    <div style={{paddingBottom:96}}>
      <Toast t={t}/>
      <PH title="Marketing" sub="Campaigns, SMS & email"/>
      <div style={{padding:"0 12px",display:"flex",flexDirection:"column",gap:12}}>

        {/* Preview teaser */}
        <div style={{background:`color-mix(in srgb,${b.theme.primary} 8%,var(--sf))`,borderRadius:16,padding:"20px 16px",border:`1.5px solid ${b.theme.primary}30`,textAlign:"center"}}>
          <div style={{fontSize:36,marginBottom:10}}>📣</div>
          <div style={{fontFamily:"var(--fd)",fontSize:18,fontWeight:700,color:"var(--tx)",marginBottom:6}}>Marketing Suite</div>
          <div style={{fontSize:13,color:"var(--mu)",lineHeight:1.6,marginBottom:16}}>Send SMS & email campaigns, build AI-powered promotions, and grow your customer base — all from BakerOS.</div>
          <button onClick={()=>setPage("subscription")} style={{width:"100%",padding:"13px",borderRadius:12,border:"none",background:b.theme.primary,color:"#fff",fontFamily:"var(--fb)",fontWeight:800,fontSize:14,cursor:"pointer"}}>Upgrade to Pro — $39.99/mo →</button>
          <div style={{fontSize:11,color:"var(--mu)",marginTop:8}}>Cancel anytime · No contracts</div>
        </div>

        {/* Feature preview cards */}
        {[
          {icon:"📱",title:"SMS Campaigns",desc:"Send targeted texts to your customer list — new products, flash sales, holiday specials."},
          {icon:"✉️",title:"Email Campaigns",desc:"Beautiful branded emails that go straight to your customers' inboxes."},
          {icon:"🤖",title:"AI Campaign Builder",desc:"Describe your goal and let AI write the SMS, email, and ad creative for you in seconds."},
          {icon:"🎯",title:"Customer Segments",desc:"Target VIP customers, repeat buyers, or NFC leads with the right message."},
          {icon:"📊",title:"Campaign Analytics",desc:"See open rates, click-throughs, and revenue from every campaign you send."},
        ].map(({icon,title,desc})=>(
          <div key={title} className="card" style={{padding:"14px 15px",opacity:.75}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              <div style={{width:40,height:40,borderRadius:11,background:`color-mix(in srgb,${b.theme.primary} 12%,var(--bg))`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>{icon}</div>
              <div style={{flex:1}}>
                <div style={{fontSize:13,fontWeight:700,color:"var(--tx)",marginBottom:2}}>{title}</div>
                <div style={{fontSize:11,color:"var(--mu)",lineHeight:1.5}}>{desc}</div>
              </div>
  ...
  if (aiView && !canAccess(mktTier, "ai_campaigns")) return (
    <GateWall feature="ai_campaigns"/>
  );
...
function ReceiptPage({ onAddToInventory, inventory, setPage: receiptSetPage }) {
  ...
  // Scan counter — persisted in baker_settings.scan_count_month (YYYY-MM format)
  // localStorage used as optimistic cache only; Supabase is the source of truth
  const getScanKey = () => {
    const now = new Date();
  ...
  // Load real scan count from Supabase on mount
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session?.user?.id) return;
      const monthKey = getScanKey().replace('bos_scans_', '');
      supabase.from('baker_settings')
        .select('scan_counts')
        .eq('user_id', session.user.id)
        .single()
        .then(({ data }) => {
          if (data?.scan_counts?.[monthKey]) {
            const serverCount = parseInt(data.scan_counts[monthKey], 10) || 0;
            setScansUsed(serverCount);
            try { localStorage.setItem(getScanKey(), String(serverCount)); } catch {}
          }
        });
    });
  }, []);
  const incrementScan = async () => {
    ...
    if (session?.user?.id) {
      const monthKey = getScanKey().replace('bos_scans_', '');
      const { data } = await supabase.from('baker_settings').select('scan_counts').eq('user_id', session.user.id).single();
      const counts = data?.scan_counts || {};
      counts[monthKey] = next;
      supabase.from('baker_settings').update({ scan_counts: counts }).eq('user_id', session.user.id);
    }
  };
...
/* ── OTHER PAGES ───────────────────────────── */
function CustomersPage({ customers=[], setCustomers=()=>{}, orders=[] }){
  const b = useBrand();
  const {tier:crmTier} = useTier();
  const crmTitle = "Customers";
  const [sel, setSel]             = useState(null);
  ...
  // All customers visible on basic Customers page regardless of tier
  // NFC lead *management features* are Pro — but NFC-captured contacts are just customers
  ...
  return (
    <div style={{paddingBottom:96}}>
      ...
      {sel && (()=>{
        const selOrders = orders.filter(o =>
          o.customer === sel.name ||
          (sel.phone && o.phone === sel.phone) ||
          (sel.email && o.email === sel.email)
        );
        const tc = TAGCOL[sel.tag] || TAGCOL.New;
        const allTags = [...new Set([
          sel.tag,
          ...(Array.isArray(sel.tags) ? sel.tags : [])
        ])].filter(Boolean);
        return (
          <div className="sheet">
            <div onClick={()=>setSel(null)} style={{position:"absolute",inset:0,background:"rgba(0,0,0,.4)",backdropFilter:"blur(2px)"}}/>
            ...
            >
              ...
              <div style={{padding:"4px 16px calc(110px + env(safe-area-inset-bottom, 0px))"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:11,marginBottom:14}}>
                  <Avt name={sel.name} sz={48}/>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontFamily:"var(--fd)",fontSize:18,fontWeight:700,color:"var(--tx)"}}>{sel.name}</div>
                    <span className="badge" style={{background:tc.bg,color:tc.c,marginTop:3,display:"inline-block"}}>{sel.tag||"New"}</span>
                  </div>
                  <button onClick={()=>setSel(null)} style={{background:"none",border:"none",fontSize:20,color:"var(--mu)",cursor:"pointer",flexShrink:0}}>×</button>
                </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:14}}>
                  {[
                    ...
                    <div key={k} style={{background:"var(--bg)",borderRadius:11,padding:"10px 11px",textAlign:"center"}}>
                      <div style={{fontSize:9,color:"var(--mu)",fontWeight:700,textTransform:"uppercase",letterSpacing:".7px",marginBottom:3}}>{k}</div>
                      <div style={{fontSize:15,fontWeight:800,color:"var(--tx)",lineHeight:1}}>{v}</div>
                    </div>
                  ))}
                </div>

                {/* Contact info */}
                <div style={{background:"var(--bg)",borderRadius:12,padding:"12px 14px",marginBottom:11}}>
                  <div style={{fontSize:10,fontWeight:700,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginBottom:10}}>📞 Contact</div>
                  {sel.phone && (
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                      <span style={{fontSize:12,color:"var(--mu)"}}>Phone</span>
                      <span style={{fontSize:13,fontWeight:600,color:"var(--tx)"}}>{sel.phone}</span>
                    </div>
                  )}
                  {sel.email && (
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                      <span style={{fontSize:12,color:"var(--mu)"}}>Email</span>
                      <span style={{fontSize:13,fontWeight:600,color:"var(--tx)",maxWidth:"60%",textAlign:"right",wordBreak:"break-all"}}>{sel.email}</span>
                    </div>
                  )}
                  {!sel.phone && !sel.email && (
                    <div style={{fontSize:12,color:"var(--mu)",fontStyle:"italic"}}>No contact info saved</div>
                  )}
                  {sel.smsOptIn !== undefined && (
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:4}}>
                      <span style={{fontSize:12,color:"var(--mu)"}}>SMS opt-in</span>
                      <span style={{fontSize:12,fontWeight:700,color:sel.smsOptIn?"#16A34A":"#9CA3AF"}}>{sel.smsOptIn?"✓ Opted in":"Not opted in"}</span>
                    </div>
                  )}
                  {sel.source === "nfc" && (
                    <div style={{marginTop:6,display:"inline-flex",alignItems:"center",gap:5,background:"#EAF0FF",borderRadius:20,padding:"2px 10px"}}>
                      <span style={{fontSize:10,fontWeight:700,color:"#1A3A8F"}}>📲 NFC Lead</span>
                    </div>
                  )}
                </div>
                ...
                {allTags.length > 0 && (
                  <div style={{background:"var(--bg)",borderRadius:12,padding:"12px 14px",marginBottom:11}}>
                    <div style={{fontSize:10,fontWeight:700,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginBottom:8}}>🏷 Tags</div>
                    <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                      {allTags.map(tg=>{
                        const tc2 = TAGCOL[tg]||{bg:"var(--bd)",c:"var(--mu)"};
                        return <span key={tg} className="badge" style={{background:tc2.bg,color:tc2.c}}>{tg}</span>;
                      })}
                    </div>
                  </div>
                )}
                ...
                {(sel.note || sel.notes) && (
                  <div style={{background:"var(--bg)",borderRadius:12,padding:"12px 14px",marginBottom:11}}>
                    <div style={{fontSize:10,fontWeight:700,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginBottom:6}}>📝 Notes</div>
                    <div style={{fontSize:13,color:"var(--tx)",lineHeight:1.6}}>{sel.note || sel.notes}</div>
                  </div>
                )}

                {/* Allergies */}
                {sel.allergies && (
                  <div style={{background:"#FEF2F2",borderRadius:12,padding:"12px 14px",marginBottom:11,border:"1px solid #FCA5A5"}}>
                    <div style={{fontSize:10,fontWeight:700,color:"#991B1B",textTransform:"uppercase",letterSpacing:".8px",marginBottom:6}}>⚠️ Allergies / Dietary</div>
                    <div style={{fontSize:13,color:"#7F1D1D",lineHeight:1.6}}>{sel.allergies}</div>
                  </div>
                )}
                ...
                {selOrders.length > 0 && (
                  <div style={{background:"var(--bg)",borderRadius:12,padding:"12px 14px",marginBottom:11}}>
                    <div style={{fontSize:10,fontWeight:700,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginBottom:10}}>🎂 Order History</div>
                    <div style={{display:"flex",flexDirection:"column",gap:8}}>
                      {selOrders.slice(0,5).map(o=>(
                        <div key={o.id} style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",paddingBottom:8,borderBottom:"1px solid var(--bd)"}}>
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{fontSize:13,fontWeight:600,color:"var(--tx)",marginBottom:2}}>{o.item}</div>
                            <div style={{fontSize:11,color:"var(--mu)"}}>{o.date}{o.dueDate?` · Due ${o.dueDate}`:""}</div>
                          </div>
                          <div style={{textAlign:"right",flexShrink:0,marginLeft:10}}>
                            <div style={{fontSize:13,fontWeight:800,color:"var(--p)"}}>${o.amount||0}</div>
                            <div style={{fontSize:10,color:"var(--mu)",textTransform:"capitalize",marginTop:1}}>{o.status}</div>
                          </div>
                        </div>
                      ))}
                      {selOrders.length > 5 && (
                        <div style={{fontSize:11,color:"var(--mu)",textAlign:"center"}}>+{selOrders.length-5} more orders</div>
                      )}
                    </div>
                  </div>
                )}
...
function AdvancedCRMPage({ customers=[], setCustomers=()=>{}, orders=[] }) {
  const b = useBrand();
  const [search, setSearch]           = useState("");
  const [filterTag, setFilterTag]     = useState("all");
  const [filterSource, setFilterSource] = useState("all");
  const [sel, setSel]                 = useState(null);
  const [editing, setEditing]         = useState(false);  // true = edit mode on detail sheet
  const [editForm, setEditForm]       = useState({});     // editable copy of sel
  const [t, show]                     = useToast();
  const [saving, setSaving]           = useState(false);

  const ALL_TAGS = ["New","Regular","VIP","Wholesale","Inactive","NFC Lead","At Risk","Repeat"];

  // Open detail — load a fresh editable copy
  const openSel = (c) => {
    setSel(c);
    setEditForm({
      tag:          c.tag || "New",
      ...
      kidsNames:    c.kidsNames || "",
      kidsBirthdays: c.kidsBirthdays || "",
  ...
  const saveEdits = async () => {
    ...
    const { data: { session } } = await supabase.auth.getSession();
    if (session?.user?.id) {
      const { error } = await supabase.from("baker_customers").update({
        tag:            editForm.tag,
        notes:          editForm.notes,
        allergies:      editForm.allergies || null,
        birthday:       editForm.birthday || null,
        anniversary:    editForm.anniversary || null,
        kids_names:     editForm.kidsNames || null,
        kids_birthdays: editForm.kidsBirthdays || null,
        custom_notes:   editForm.customNotes || null,
        updated_at:     new Date().toISOString(),
      }).eq("id", sel.id).eq("baker_id", session.user.id);
      if (error) { show("Save failed — " + error.message, "error"); }
      else { show("Customer updated ✓"); }
    }
    setSaving(false);
  };
  ...
  const filtered = customers.filter(c => {
    ...
    const allTags = [c.tag, ...(Array.isArray(c.tags) ? c.tags : [])].filter(Boolean);
    const matchTag = filterTag === "all" || allTags.includes(filterTag);
    const matchSource = filterSource === "all" ||
      (filterSource === "nfc" && (c.source === "nfc" || allTags.includes("NFC Lead"))) ||
      (filterSource === "manual" && c.source !== "nfc");
    return matchSearch && matchTag && matchSource;
  });

  const nfcLeads   = customers.filter(c => c.source === "nfc" || (c.tags||[]).includes("NFC Lead"));
  const vipCount   = customers.filter(c => c.tag === "VIP").length;
  const repeatCount = customers.filter(c => (c.orders||0) > 1).length;
  ...
  return (
    <div style={{paddingBottom:96}}>
      ...
      <PH
        title="Advanced CRM"
        sub={`${customers.length} customers · ${nfcLeads.length} NFC leads`}
        action={<PageHelp pageKey="advanced_crm"/>}
      />

      {/* Stats */}
      <div style={{padding:"0 12px 12px",display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
        {[["👥",customers.length,"Total"],["⭐",vipCount,"VIP"],["📲",nfcLeads.length,"NFC"]].map(([ic,val,lbl])=>(
          <div key={lbl} className="card" style={{padding:"11px 10px",textAlign:"center"}}>
            <div style={{fontSize:18}}>{ic}</div>
            <div style={{fontSize:18,fontWeight:800,color:"var(--tx)",lineHeight:1,marginTop:3}}>{val}</div>
      ...
      <div style={{padding:"0 12px 10px",display:"flex",flexDirection:"column",gap:8}}>
        <input className="field" placeholder="Search name, phone, or email..." value={search} onChange={e=>setSearch(e.target.value)} style={{fontSize:13}}/>
        <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:2}}>
          {["all",...ALL_TAGS].map(tag=>(
            <button key={tag} onClick={()=>setFilterTag(tag)}
              style={{flexShrink:0,padding:"5px 12px",borderRadius:20,border:`1.5px solid ${filterTag===tag?b.theme.primary:"var(--bd)"}`,background:filterTag===tag?b.theme.primary:"var(--sf)",color:filterTag===tag?"#fff":"var(--mu)",fontSize:12,fontWeight:filterTag===tag?700:500,cursor:"pointer",fontFamily:"var(--fb)"}}>
              {tag==="all"?"All":tag}
            </button>
          ))}
        </div>
        <div style={{display:"flex",gap:6}}>
          {[["all","All Sources"],["nfc","NFC Leads"],["manual","Manual"]].map(([val,lbl])=>(
            <button key={val} onClick={()=>setFilterSource(val)}
              style={{padding:"5px 12px",borderRadius:20,border:`1.5px solid ${filterSource===val?b.theme.primary:"var(--bd)"}`,background:filterSource===val?b.theme.primary:"var(--sf)",color:filterSource===val?"#fff":"var(--mu)",fontSize:12,fontWeight:filterSource===val?700:500,cursor:"pointer",fontFamily:"var(--fb)"}}>
              {lbl}
            </button>
          ))}
        </div>
      ...
      <div style={{padding:"0 12px",display:"flex",flexDirection:"column",gap:8}}>
        {filtered.length===0 ? (
          <div className="card" style={{padding:"28px",textAlign:"center"}}>
            ...
            <div style={{fontSize:14,fontWeight:700,color:"var(--tx)",marginBottom:4}}>No customers match</div>
            <div style={{fontSize:12,color:"var(--mu)"}}>Try a different search or filter</div>
          </div>
        ) : filtered.map(c=>{
          const allTags=[...new Set([c.tag,...(Array.isArray(c.tags)?c.tags:[])].filter(Boolean))];
          const isNFC=c.source==="nfc"||allTags.includes("NFC Lead");
          const hasAllergy=!!c.allergies;
          return (
            <div key={c.id} className="card row" style={{padding:"12px 13px",cursor:"pointer"}} onClick={()=>openSel(c)}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <Avt name={c.name} sz={40}/>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3,flexWrap:"wrap"}}>
                    <span style={{fontSize:13,fontWeight:700,color:"var(--tx)"}}>{c.name}</span>
                    {isNFC&&<span style={{fontSize:9,fontWeight:700,background:"#EAF0FF",color:"#1A3A8F",borderRadius:20,padding:"1px 6px"}}>📲 NFC</span>}
                    {hasAllergy&&<span style={{fontSize:9,fontWeight:700,background:"#FEE2E2",color:"#991B1B",borderRadius:20,padding:"1px 6px"}}>⚠️</span>}
                    {c.birthday&&<span style={{fontSize:9,fontWeight:700,background:"#FFF0CC",color:"#7A4F00",borderRadius:20,padding:"1px 6px"}}>🎂</span>}
                  </div>
                  <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:3}}>
                    {allTags.slice(0,3).map(tag=>{const tc2=TAGCOL[tag]||{bg:"var(--bd)",c:"var(--mu)"};return <span key={tag} className="badge" style={{background:tc2.bg,color:tc2.c,fontSize:9}}>{tag}</span>;})}
                  </div>
                  <div style={{fontSize:11,color:"var(--mu)"}}>
                    {c.orders||0} order{(c.orders||0)!==1?"s":""} · ${c.spent||0}{c.phone?` · ${c.phone}`:""}
                  </div>
                </div>
                <span style={{fontSize:14,color:"var(--mu)"}}>›</span>
              </div>
      ...
      {sel && (()=>{
        ...
        const allTags=[...new Set([sel.tag,...(Array.isArray(sel.tags)?sel.tags:[])].filter(Boolean))];
        return (
          <div className="sheet">
            <div onClick={()=>setSel(null)} style={{position:"absolute",inset:0,background:"rgba(0,0,0,.4)",backdropFilter:"blur(2px)"}}/>
            <div className="sheet-panel" style={{maxHeight:"92dvh",overflowY:"auto",WebkitOverflowScrolling:"touch",touchAction:"pan-y",paddingBottom:0}}>
              ...
              <div style={{padding:"4px 16px calc(110px + env(safe-area-inset-bottom,0px))"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:11,marginBottom:14}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontFamily:"var(--fd)",fontSize:18,fontWeight:700,color:"var(--tx)"}}>{sel.name}</div>
                    <div style={{fontSize:11,color:"var(--mu)",marginTop:2}}>{sel.phone||""}{sel.email?` · ${sel.email}`:""}</div>
                  </div>
                  <div style={{display:"flex",gap:6,flexShrink:0}}>
                    {!editing&&<button onClick={()=>setEditing(true)} style={{background:b.theme.primary,color:"#fff",border:"none",borderRadius:9,padding:"7px 13px",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"var(--fb)"}}>Edit</button>}
                    <button onClick={()=>setSel(null)} style={{background:"none",border:"none",fontSize:20,color:"var(--mu)",cursor:"pointer"}}>×</button>
                  </div>
                </div>
                ...
                {/* Stats */}
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:14}}>
                  {[["Orders",sel.orders||0],["Spent","$"+(sel.spent||0)],["Last",sel.last||"Never"]].map(([k,v])=>(
                    <div key={k} style={{background:"var(--bg)",borderRadius:10,padding:"9px 10px",textAlign:"center"}}>
                      <div style={{fontSize:9,color:"var(--mu)",fontWeight:700,textTransform:"uppercase",letterSpacing:".7px",marginBottom:2}}>{k}</div>
                ...
                {!editing && (<>
                  ...
                  {/* Tag */}
                  <div style={{background:"var(--bg)",borderRadius:11,padding:"11px 13px",marginBottom:10}}>
                    <div style={{fontSize:10,fontWeight:700,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginBottom:6}}>🏷 Status & Tags</div>
                    <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                      {allTags.map(tag=>{const tc2=TAGCOL[tag]||{bg:"var(--bd)",c:"var(--mu)"};return <span key={tag} className="badge" style={{background:tc2.bg,color:tc2.c}}>{tag}</span>;})}
                    </div>
                  </div>
                  ...
                  {(sel.phone||sel.email)&&(
                    <div style={{background:"var(--bg)",borderRadius:11,padding:"11px 13px",marginBottom:10}}>
                      <div style={{fontSize:10,fontWeight:700,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginBottom:8}}>📞 Contact</div>
                      {sel.phone&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}><span style={{fontSize:12,color:"var(--mu)"}}>Phone</span><span style={{fontSize:13,fontWeight:600,color:"var(--tx)"}}>{sel.phone}</span></div>}
                      {sel.email&&<div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontSize:12,color:"var(--mu)"}}>Email</span><span style={{fontSize:12,fontWeight:600,color:"var(--tx)",maxWidth:"60%",textAlign:"right",wordBreak:"break-all"}}>{sel.email}</span></div>}
                      {sel.source==="nfc"&&<div style={{marginTop:7}}><span style={{fontSize:10,fontWeight:700,background:"#EAF0FF",color:"#1A3A8F",borderRadius:20,padding:"2px 8px"}}>📲 NFC Lead</span></div>}
                    </div>
                  )}

                  {/* Key dates */}
                  {(sel.birthday||sel.anniversary||sel.kidsNames||sel.kidsBirthdays)&&(
                    <div style={{background:"var(--bg)",borderRadius:11,padding:"11px 13px",marginBottom:10}}>
                      <div style={{fontSize:10,fontWeight:700,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginBottom:8}}>📅 Key Dates</div>
                      {sel.birthday&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}><span style={{fontSize:12,color:"var(--mu)"}}>🎂 Birthday</span><span style={{fontSize:13,fontWeight:600,color:"var(--tx)"}}>{sel.birthday}</span></div>}
                      {sel.anniversary&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}><span style={{fontSize:12,color:"var(--mu)"}}>💍 Anniversary</span><span style={{fontSize:13,fontWeight:600,color:"var(--tx)"}}>{sel.anniversary}</span></div>}
                      {sel.kidsNames&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}><span style={{fontSize:12,color:"var(--mu)"}}>👶 Kids</span><span style={{fontSize:13,fontWeight:600,color:"var(--tx)",maxWidth:"60%",textAlign:"right"}}>{sel.kidsNames}</span></div>}
                      {sel.kidsBirthdays&&<div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontSize:12,color:"var(--mu)"}}>🎁 Kids' Bdays</span><span style={{fontSize:13,fontWeight:600,color:"var(--tx)",maxWidth:"60%",textAlign:"right"}}>{sel.kidsBirthdays}</span></div>}
                    </div>
                  )}

                  {/* Allergies */}
                  {sel.allergies&&<div style={{background:"#FEF2F2",borderRadius:11,padding:"11px 13px",marginBottom:10,border:"1px solid #FCA5A5"}}><div style={{fontSize:10,fontWeight:700,color:"#991B1B",textTransform:"uppercase",letterSpacing:".8px",marginBottom:5}}>⚠️ Allergies / Dietary</div><div style={{fontSize:13,color:"#7F1D1D",lineHeight:1.6}}>{sel.allergies}</div></div>}

                  {/* Notes */}
                  {(sel.notes||sel.note)&&<div style={{background:"var(--bg)",borderRadius:11,padding:"11px 13px",marginBottom:10}}><div style={{fontSize:10,fontWeight:700,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginBottom:5}}>📝 Notes</div><div style={{fontSize:13,color:"var(--tx)",lineHeight:1.6}}>{sel.notes||sel.note}</div></div>}

                  {/* Custom notes */}
                  {sel.customNotes&&<div style={{background:"var(--bg)",borderRadius:11,padding:"11px 13px",marginBottom:10}}><div style={{fontSize:10,fontWeight:700,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginBottom:5}}>✨ Baker Notes</div><div style={{fontSize:13,color:"var(--tx)",lineHeight:1.6}}>{sel.customNotes}</div></div>}

                  {/* No info prompt */}
                  {!sel.birthday&&!sel.anniversary&&!sel.kidsNames&&!sel.allergies&&!(sel.notes||sel.note)&&!sel.customNotes&&(
                    <div style={{background:`color-mix(in srgb,${b.theme.primary} 6%,var(--bg))`,borderRadius:11,padding:"12px 14px",marginBottom:10,border:`1px dashed ${b.theme.primary}40`,textAlign:"center"}}>
                      <div style={{fontSize:12,color:"var(--mu)",lineHeight:1.5}}>Tap <strong>Edit</strong> to add birthdays, allergies, notes, and more — the more you know, the better you can serve them 🎂</div>
                    </div>
                  )}
                  ...
                  {selOrders.length>0&&(
                    <div style={{background:"var(--bg)",borderRadius:11,padding:"11px 13px",marginBottom:10}}>
                      <div style={{fontSize:10,fontWeight:700,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginBottom:8}}>🎂 Order History</div>
                      <div style={{display:"flex",flexDirection:"column",gap:7}}>
                        {selOrders.slice(0,5).map(o=>(
                          <div key={o.id} style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",paddingBottom:7,borderBottom:"1px solid var(--bd)"}}>
                            <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:"var(--tx)"}}>{o.item}</div><div style={{fontSize:11,color:"var(--mu)"}}>{o.date}{o.dueDate?` · Due ${o.dueDate}`:""}</div></div>
                            <div style={{textAlign:"right",marginLeft:10}}><div style={{fontSize:13,fontWeight:800,color:"var(--p)"}}>${o.amount||0}</div><div style={{fontSize:10,color:"var(--mu)",textTransform:"capitalize"}}>{o.status}</div></div>
                          </div>
                        ))}
                        {selOrders.length>5&&<div style={{fontSize:11,color:"var(--mu)",textAlign:"center"}}>+{selOrders.length-5} more orders</div>}
                      </div>
                    </div>
                  )}
                  ...
                  {/* Actions */}
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                    <button className="pbtn" onClick={()=>{if(sel.phone)window.open(`sms:${sel.phone}`,"_blank");else show("No phone saved");}}>📱 SMS</button>
                    <button onClick={()=>{if(!window.confirm(`Remove ${sel.name}?`))return;const id=sel.id;setCustomers(p=>p.filter(c=>c.id!==id));setSel(null);supabase.auth.getSession().then(({data:{session}})=>{if(session?.user?.id)supabase.from("baker_customers").delete().eq("id",id).eq("baker_id",session.user.id);});show("Removed");}} style={{padding:"13px",borderRadius:12,color:"#DC2626",border:"1.5px solid #FCA5A5",background:"#FEF2F2",fontFamily:"var(--fb)",fontWeight:600,fontSize:13,cursor:"pointer"}}>Remove</button>
                  </div>
                ...
                {editing && (
                  <div style={{display:"flex",flexDirection:"column",gap:12}}>
                    ...
                    <Fld label="Status / Tag">
                      <select className="field" value={editForm.tag} onChange={e=>setEditForm(p=>({...p,tag:e.target.value}))}>
                        {ALL_TAGS.map(t=><option key={t} value={t}>{t}</option>)}
                      </select>
                    </Fld>

                    <Fld label="Allergies / Dietary Restrictions">
                      <input className="field" placeholder="e.g. Peanut allergy, gluten-free" value={editForm.allergies} onChange={e=>setEditForm(p=>({...p,allergies:e.target.value}))}/>
                    </Fld>

                    <div style={{fontSize:11,fontWeight:700,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginTop:4}}>📅 Key Dates</div>

                    <Fld label="Birthday">
                      <input className="field" placeholder="e.g. March 15" value={editForm.birthday} onChange={e=>setEditForm(p=>({...p,birthday:e.target.value}))}/>
                    </Fld>

                    <Fld label="Anniversary">
                      <input className="field" placeholder="e.g. June 22" value={editForm.anniversary} onChange={e=>setEditForm(p=>({...p,anniversary:e.target.value}))}/>
                    </Fld>

                    <Fld label="Kids' Names">
                      <input className="field" placeholder="e.g. Emma, Jake" value={editForm.kidsNames} onChange={e=>setEditForm(p=>({...p,kidsNames:e.target.value}))}/>
                    </Fld>

                    <Fld label="Kids' Birthdays">
                      <input className="field" placeholder="e.g. Emma - Apr 3, Jake - Sep 12" value={editForm.kidsBirthdays} onChange={e=>setEditForm(p=>({...p,kidsBirthdays:e.target.value}))}/>
                    </Fld>

                    <div style={{fontSize:11,fontWeight:700,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginTop:4}}>📝 Notes</div>

                    <Fld label="Customer Notes">
                      <textarea className="field" rows={3} placeholder="Preferences, flavor favorites, past orders they loved..." value={editForm.notes} onChange={e=>setEditForm(p=>({...p,notes:e.target.value}))} style={{resize:"none"}}/>
                    </Fld>

                    <Fld label="Baker Notes (private)">
                      <textarea className="field" rows={3} placeholder="Anything you want to remember — pricing notes, custom requests, how you met them..." value={editForm.customNotes} onChange={e=>setEditForm(p=>({...p,customNotes:e.target.value}))} style={{resize:"none"}}/>
                    </Fld>

                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:4}}>
                      <button className="pbtn" onClick={saveEdits} disabled={saving}>
                        {saving?"Saving...":"Save Changes"}
                      </button>
                      <button className="gbtn" onClick={()=>setEditing(false)} style={{padding:"13px",borderRadius:12,color:"var(--tx)"}}>Cancel</button>
                    </div>
...
function InvoicePage({ payHandles, products=[], invoices=[], setInvoices=()=>{}, customers=[], setCustomers=()=>{} }) {
  ...
  const [form, setForm] = useState({customer:"", email:"", phone:"", lineItems:[{productId:"", name:"", qty:1, price:""}], due:"", dueDate:"", imageURL:null, depositType:"percent", depositValue:""});
  ...
  const createInvoice = () => {
    ...
    const depositAmt = form.depositValue
      ? (form.depositType==="percent"
          ? parseFloat((lineTotal * (parseFloat(form.depositValue)||0) / 100).toFixed(2))
          : parseFloat(parseFloat(form.depositValue||0).toFixed(2)))
      : 0;
    const newInv = {
      ...
      balance:       parseFloat((lineTotal - depositAmt).toFixed(2)),
      status:        "unpaid",
      date:          "Today",
      due:           form.due || "Net 7",
    ...
    supabase.auth.getSession().then(({data:{session}})=>{
      if(session?.user?.id) supabase.from("baker_invoices").insert({
        ...
        phone: newInv.phone||null, deposit_amount: newInv.depositAmount||0,
        balance: newInv.balance||0,
      }).then(({error})=>{ if(error) console.error('[invoice insert]', error.message); });
    });
    setForm({customer:"", email:"", phone:"", lineItems:[{productId:"", name:"", qty:1, price:""}], due:"", dueDate:"", imageURL:null, depositType:"percent", depositValue:""});
    setShowNew(false);
  ...
  const markDepositReceived = async (id) => {
    setInvoices(p => p.map(i => i.id===id ? {...i, depositReceived:true, status: i.balance===0 ? "paid" : i.status} : i));
    setSel(p => p ? {...p, depositReceived:true, status: p.balance===0 ? "paid" : p.status} : p);
    show("Deposit received ✓");
    const { data: { session } } = await supabase.auth.getSession();
    if (session?.user?.id) {
      supabase.from("baker_invoices")
        .update({ deposit_received: true, updated_at: new Date().toISOString() })
        .eq("id", id).eq("baker_id", session.user.id);
    }
  };
  ...
  const deleteInvoice = async (id) => {
    ...
    const { data: { session } } = await supabase.auth.getSession();
    if (session?.user?.id) {
      supabase.from("baker_invoices").delete().eq("id", id).eq("baker_id", session.user.id);
    }
  };

  const openSend = () => { setSendStep("choose"); setSendTo(""); setShowSend(true); setShowDeleteConfirm(false); };
      ...
      <div style="font-family:'Helvetica Neue',sans-serif;max-width:520px;margin:0 auto;background:#FFFBF5;border-radius:16px;overflow:hidden;border:1px solid #E8D5C0;">
        ...
        <div style="padding:28px 32px;">
          ...
          <div style="margin-bottom:24px;">
            <div style="font-size:12px;font-weight:700;color:#9C7B5C;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;">📸 Your Order</div>
            <img src="${sel.finalImageURL}" alt="Your order" style="width:100%;border-radius:12px;display:block;border:1px solid #E8D5C0;"/>
          </div>` : ''}
          ...
          <div className="sheet-panel" style={{paddingBottom:"calc(96px + env(safe-area-inset-bottom, 16px))"}}>
            ...
            <div style={{padding:"6px 16px 16px"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9,marginBottom:14}}>
                {[["Items",sel.items],["Total","$"+(sel.amount||0)],["Due Date",sel.due],["Status",sel.status]].map(([k,v])=>(
                  <div key={k} style={{background:"var(--bg)",borderRadius:10,padding:"9px 11px"}}>
                    <div style={{fontSize:9,color:"var(--mu)",fontWeight:700,textTransform:"uppercase",letterSpacing:".7px",marginBottom:2}}>{k}</div>
              ...
              {/* Deposit breakdown — shown if invoice has a deposit */}
              {sel.depositAmount > 0 && (
                <div style={{background:`color-mix(in srgb,${b.theme.primary} 6%,var(--bg))`,borderRadius:13,padding:"13px 15px",marginBottom:14,border:`1px solid ${b.theme.primary}30`}}>
                  <div style={{fontSize:10,fontWeight:800,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginBottom:10}}>💰 Payment Breakdown</div>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                    <span style={{fontSize:13,color:"var(--mu)"}}>Deposit due now</span>
                    <span style={{fontSize:14,fontWeight:800,color:b.theme.primary}}>${(sel.depositAmount||0).toFixed(2)}</span>
                  </div>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                    <span style={{fontSize:13,color:"var(--mu)"}}>Balance due {sel.due||"by due date"}</span>
                    <span style={{fontSize:14,fontWeight:700,color:"var(--tx)"}}>${(sel.balance||(sel.amount-(sel.depositAmount||0))).toFixed(2)}</span>
                  </div>
                  <div style={{borderTop:"1px solid var(--bd)",paddingTop:8,display:"flex",justifyContent:"space-between"}}>
                    ...
                    <span style={{fontSize:15,fontWeight:800,color:"var(--tx)"}}>${(sel.amount||0).toFixed(2)}</span>
                  </div>
                </div>
              )}
              ...
              {/* Deposit Received button — only shown if invoice has a deposit and it hasn't been received yet */}
              {sel.depositAmount > 0 && !sel.depositReceived && (
                <div style={{marginBottom:14,background:"#D1ECE4",borderRadius:13,padding:"13px 15px",border:"1px solid #A3CFBB"}}>
                  <div style={{fontSize:12,fontWeight:700,color:"#155724",marginBottom:6}}>💰 Deposit of ${(sel.depositAmount||0).toFixed(2)} pending</div>
                  <div style={{fontSize:11,color:"#155724",marginBottom:10,opacity:.8}}>Tap below once you've received the deposit payment from {sel.customer}.</div>
                  <button
                    onClick={()=>markDepositReceived(sel.id)}
                    style={{width:"100%",padding:"11px",borderRadius:11,border:"none",background:"#155724",color:"#fff",fontFamily:"var(--fb)",fontWeight:800,fontSize:13,cursor:"pointer"}}
                  >
              ...
              {sel.depositAmount > 0 && sel.depositReceived && (
                <div style={{marginBottom:14,background:"#D1ECE4",borderRadius:13,padding:"11px 14px",border:"1px solid #A3CFBB",display:"flex",alignItems:"center",gap:10}}>
                  <span style={{fontSize:18}}>✅</span>
                  <div>
                    <div style={{fontSize:12,fontWeight:800,color:"#155724"}}>Deposit received</div>
                    <div style={{fontSize:11,color:"#155724",opacity:.8}}>Balance due: ${(sel.balance||(sel.amount-(sel.depositAmount||0))).toFixed(2)}</div>
                  </div>
                </div>
              )}
              ...
              {!showDeleteConfirm ? (
                ...
                <div style={{marginTop:10,background:"#FEF2F2",borderRadius:12,padding:"13px 14px"}}>
                  <div style={{fontSize:13,fontWeight:700,color:"#DC2626",marginBottom:10,textAlign:"center"}}>Delete this invoice?</div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
                    <button onClick={()=>setShowDeleteConfirm(false)} style={{padding:"10px",borderRadius:10,border:"1px solid var(--bd)",background:"var(--sf)",color:"var(--mu)",fontFamily:"var(--fb)",fontWeight:600,fontSize:13,cursor:"pointer"}}>Cancel</button>
                    <button onClick={()=>deleteInvoice(sel.id)} style={{padding:"10px",borderRadius:10,border:"none",background:"#DC2626",color:"#fff",fontFamily:"var(--fb)",fontWeight:700,fontSize:13,cursor:"pointer"}}>Yes, Delete</button>
                  </div>
                </div>
              )}
            ...
            <div style={{padding:"0 16px",display:"flex",flexDirection:"column",gap:13}}>
              ...
              <Fld label="Due Date">
                <input
                  ...
                  onChange={e=>{
                    ...
                    setForm(p=>({...p,due:formatted,dueDate:e.target.value}));
                  }}
                  style={{colorScheme:"light"}}
                />
                {form.due && <div style={{fontSize:11,color:"var(--mu)",marginTop:4}}>📅 Due {form.due}</div>}
              </Fld>
              ...
              {(()=>{
                ...
                const depositAmt = form.depositType==="percent"
                  ? (lineTotal * (parseFloat(form.depositValue)||0) / 100)
                  : (parseFloat(form.depositValue)||0);
                const balance = lineTotal - depositAmt;
                return (
                  <div style={{background:`color-mix(in srgb,${b.theme.primary} 6%,var(--bg))`,borderRadius:14,padding:"14px",border:`1px solid ${b.theme.primary}30`}}>
                    <div style={{fontSize:12,fontWeight:800,color:"var(--tx)",marginBottom:10}}>💰 Deposit Required</div>
                    <div style={{display:"grid",gridTemplateColumns:"auto 1fr",gap:8,marginBottom:10}}>
                      {/* Type toggle */}
                      <div style={{display:"flex",borderRadius:9,overflow:"hidden",border:"1.5px solid var(--bd)"}}>
                        {["percent","dollar"].map(t=>(
                          <button key={t} onClick={()=>setForm(p=>({...p,depositType:t}))}
                            style={{padding:"9px 13px",border:"none",background:form.depositType===t?b.theme.primary:"var(--sf)",color:form.depositType===t?"#fff":"var(--mu)",fontFamily:"var(--fb)",fontWeight:700,fontSize:13,cursor:"pointer"}}>
                            {t==="percent"?"%":"$"}
                          </button>
                        ))}
                      </div>
                      ...
                      <input
                        ...
                        placeholder={form.depositType==="percent"?"e.g. 50":"e.g. 42.50"}
                        value={form.depositValue}
                        onChange={e=>setForm(p=>({...p,depositValue:e.target.value}))}
                      />
                    </div>
                    {/* Live breakdown */}
                    {lineTotal > 0 && parseFloat(form.depositValue) > 0 && (
                      <div style={{background:"var(--bg)",borderRadius:10,padding:"10px 12px",display:"flex",flexDirection:"column",gap:6}}>
                        <div style={{display:"flex",justifyContent:"space-between",fontSize:12}}>
                          <span style={{color:"var(--mu)"}}>Deposit due now</span>
                          <span style={{fontWeight:800,color:b.theme.primary}}>${depositAmt.toFixed(2)}</span>
                        </div>
                        <div style={{display:"flex",justifyContent:"space-between",fontSize:12}}>
                          <span style={{color:"var(--mu)"}}>Balance due {form.due||"by due date"}</span>
                          <span style={{fontWeight:700,color:"var(--tx)"}}>${balance.toFixed(2)}</span>
                        </div>
                        <div style={{borderTop:"1px solid var(--bd)",paddingTop:6,display:"flex",justifyContent:"space-between",fontSize:13}}>
                          <span style={{fontWeight:800,color:"var(--tx)"}}>Total</span>
                          <span style={{fontWeight:800,color:"var(--tx)"}}>${lineTotal.toFixed(2)}</span>
                        </div>
                      </div>
                    )}
                    <button onClick={()=>setForm(p=>({...p,depositValue:"",depositType:"percent"}))}
                      style={{background:"none",border:"none",fontSize:11,color:"var(--mu)",cursor:"pointer",marginTop:6,padding:0,fontFamily:"var(--fb)"}}>
                      Clear deposit
                    </button>
                  </div>
                );
...
function CalendarPage({ orders=[], reminders=[], setReminders=()=>{} }) {
  ...
  const canMarketingReminder = canAccess(calTier||"starter", "holiday_marketing"); // Growth+
  const [t, show] = useToast();
  ...
  const upcomingMarketingAlerts = useMemo(() => {
    const alerts = [];
    HOLIDAYS.forEach(h => {
      const [_hy,_hm,_hd] = h.date.split("-").map(Number);
      const hDate  = new Date(_hy, _hm-1, _hd);
    ...
    <div style={{paddingBottom:96}}>
      ...
      {monthHolidays.length > 0 && (
        <div style={{padding:"0 12px 12px"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {monthHolidays.map(h => {
              const [_rhy,_rhm,_rhd] = h.date.split("-").map(Number);
              const hDate = new Date(_rhy, _rhm-1, _rhd);
            ...
            <div style={{padding:"4px 16px 0"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
                ...
                <div>
                  ...
                  <div style={{fontSize:12,color:"var(--mu)",marginTop:2}}>{(()=>{const [y,m,d]=holidayPopup.date.split("-").map(Number);return new Date(y,m-1,d).toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"});})()}</div>
                </div>
              ...
              {/* Days until */}
              {(() => {
                const [_hy,_hm,_hd] = holidayPopup.date.split("-").map(Number);
                const holidayLocal = new Date(_hy, _hm-1, _hd);
                const daysAway = Math.round((holidayLocal - today) / (1000*60*60*24));
...
/* ── PRODUCTS PAGE ─────────────────────────── */
function ProductFormSheet({ isEdit, onClose, productForm, setProductForm, categories, products, recipes=[], inventory=[], SELL_BY_OPTIONS, MIN_ORDER_OPTIONS, openAddCat, toggleProductCat, catColor, catEmoji, catName, editingProduct, saveProduct, productImgRef, deleteProduct }) {
  const b = useBrand();
  const [catDropOpen, setCatDropOpen] = useState(false);
  const { tier: pfTier, setPage: pfSetPage } = useTier();  // moved from inline IIFE — hook must be at top level
        ...
        <div style={{background:"var(--bg)",borderRadius:12,padding:"12px 14px",border:"1.5px solid var(--bd)"}}>
          ...
          {!hasInv ? (
            <div style={{textAlign:"center",padding:"8px 0"}}>
              <div style={{fontSize:12,color:"var(--mu)",marginBottom:8}}>🔒 Upgrade to Pro to link recipes and auto-deduct inventory on order completion</div>
            ...
            <>
              ...
              {/* ── Profit margin card — shows when recipe is linked and price is set ── */}
              {productForm.recipeId && parseFloat(productForm.price) > 0 && (()=>{
                const linkedRecipe = recipes.find(r => r.id === productForm.recipeId);
                if (!linkedRecipe || !linkedRecipe.ingredients?.length) return null;

                // Inline cost calc (mirrors RecipePage.calcCost)
                let ingredientCost = 0;
                let missingCost = false;
                linkedRecipe.ingredients.forEach(ing => {
                  const item = inventory.find(i => i.id === ing.inventoryId);
                  if (!item) return;
                  if (!item.cost) { missingCost = true; return; }
                  ingredientCost += (parseFloat(ing.qty)||0) * (parseFloat(item.cost)||0);
                });
                const laborCost  = ((parseFloat(linkedRecipe.laborMins)||0) / 60) * (parseFloat(linkedRecipe.laborRate)||15);
                const packCost   = parseFloat(linkedRecipe.packagingCost)||0;
                const totalBatch = ingredientCost + laborCost + packCost;
                const costPerUnit = totalBatch / (parseFloat(linkedRecipe.batchYield)||1);
                const sellingPrice = parseFloat(productForm.price)||0;
                const profit = sellingPrice - costPerUnit;
                const margin = sellingPrice > 0 ? (profit / sellingPrice) * 100 : 0;
                const marginColor = margin >= 50 ? "#16A34A" : margin >= 30 ? "#C47B00" : "#DC2626";
                const marginLabel = margin >= 60 ? "Excellent" : margin >= 40 ? "Good" : margin >= 20 ? "Tight" : "Below cost risk";

                return (
                  <div style={{marginTop:10,background:`color-mix(in srgb,${marginColor} 8%,var(--sf))`,borderRadius:11,padding:"12px 13px",border:`1px solid ${marginColor}30`}}>
                    <div style={{fontSize:10,fontWeight:800,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginBottom:9}}>💰 Profit Margin</div>
                    <div style={{display:"flex",flexDirection:"column",gap:5}}>
                      <div style={{display:"flex",justifyContent:"space-between"}}>
                        <span style={{fontSize:12,color:"var(--mu)"}}>Cost per {linkedRecipe.yieldUnit||"unit"}</span>
                        <span style={{fontSize:13,fontWeight:700,color:"var(--tx)"}}>${costPerUnit.toFixed(2)}</span>
                      </div>
                      <div style={{display:"flex",justifyContent:"space-between"}}>
                        <span style={{fontSize:12,color:"var(--mu)"}}>Selling price</span>
                        <span style={{fontSize:13,fontWeight:700,color:"var(--tx)"}}>${sellingPrice.toFixed(2)}</span>
                      </div>
                      <div style={{display:"flex",justifyContent:"space-between",borderTop:"1px solid var(--bd)",paddingTop:6,marginTop:2}}>
                        <span style={{fontSize:12,fontWeight:700,color:"var(--tx)"}}>Profit per {linkedRecipe.yieldUnit||"unit"}</span>
                        <span style={{fontSize:14,fontWeight:800,color:marginColor}}>${profit.toFixed(2)}</span>
                      </div>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        <span style={{fontSize:12,fontWeight:700,color:"var(--tx)"}}>Gross margin</span>
                        <div style={{textAlign:"right"}}>
                          <span style={{fontSize:18,fontWeight:900,color:marginColor}}>{margin.toFixed(0)}%</span>
                          <span style={{fontSize:10,fontWeight:700,color:marginColor,marginLeft:6}}>{marginLabel}</span>
                        </div>
                      </div>
                    </div>
                    {missingCost && <div style={{fontSize:10,color:"#C47B00",marginTop:7}}>⚠ Some ingredients have no cost set — add in Inventory for full accuracy</div>}
                  </div>
                );
              })()}
            </>
...
function ProductsPage({ products, setProducts, categories, setCategories, tier="starter", recipes=[] }) {
  ...
  const saveProduct = (isEdit) => {
    ...
    if (!isEdit && !canAccess(tier,"unlimited_products") && products.length >= 15) {
      show("Free plan allows up to 15 products. Upgrade to Growth for unlimited.","error"); return;
    }
...
function GalleryPage({ photos=[], setPhotos=()=>{}, albums=[], setAlbums=()=>{}, products=[] }) {
  ...
  // Compress image to JPEG at 80% quality, max 1200px wide — reduces upload size ~70-90%
  ...
  const handleUpload = async file => {
    ...
    if (isVideo && file.size > maxVideoSize) {
      show("Video must be under 25MB — please compress it first", "error");
      return;
    }
    setUploading(true);
    try {
      ...
      if (!isVideo) {
        // Compress before upload — converts to JPEG regardless of source format
      ...
      const { data, error } = await supabase.storage.from("baker-assets").upload(path, uploadFile, { upsert:false, contentType });
      if (error) throw error;
      const { data: { publicUrl } } = supabase.storage.from("baker-assets").getPublicUrl(data.path);
      setNewForm(p => ({...p, url:publicUrl, type:isVideo?"video":"photo", mimeType:contentType}));
...
function StorefrontMessageForm({ onSent, bakerPhone="", bakerId="" }) {
  ...
  const handleMarkPaid = async () => {
    ...
    try {
      await fetch("/api/storefront", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type: "mark_paid", invoiceId: inv.id }),
      });
      setMarkedPaid(true);
  ...
  return (
    <div style={{minHeight:"100vh",background:"#FFFBF5",fontFamily:"'Outfit',sans-serif",padding:"0 0 48px"}}>
      ...
      <div style={{maxWidth:480,margin:"0 auto",padding:"20px 16px"}}>
        ...
        <div style={{background:"#fff",borderRadius:16,padding:"18px",marginBottom:16,border:"1px solid #E8D5C0"}}>
          ...
          {inv.depositAmount > 0 ? (
            <div style={{paddingTop:4}}>
              {inv.depositReceived ? (
                <>
                  <div style={{background:"#D1ECE4",borderRadius:10,padding:"10px 13px",marginBottom:10,display:"flex",alignItems:"center",gap:8,border:"1px solid #A3CFBB"}}>
                    <span style={{fontSize:16}}>✅</span>
                    <div>
                      <div style={{fontSize:12,fontWeight:800,color:"#155724"}}>Deposit received</div>
                      <div style={{fontSize:11,color:"#155724",opacity:.8}}>${(inv.depositAmount||0).toFixed(2)} paid</div>
                    </div>
                  </div>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",paddingTop:4}}>
                    <span style={{fontSize:14,fontWeight:800,color:"#2C1A0E"}}>Balance Due {inv.due||""}</span>
                    <span style={{fontSize:24,fontWeight:800,color:primary}}>${(inv.balance||(inv.amount-(inv.depositAmount||0))).toFixed(2)}</span>
                  </div>
                </>
              ) : (
                <>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                    <span style={{fontSize:13,color:"#9C7B5C"}}>Deposit due now</span>
                    <span style={{fontSize:16,fontWeight:800,color:primary}}>${(inv.depositAmount||0).toFixed(2)}</span>
                  </div>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}>
                    <span style={{fontSize:13,color:"#9C7B5C"}}>Balance due {inv.due||"by due date"}</span>
                    <span style={{fontSize:15,fontWeight:700,color:"#2C1A0E"}}>${(inv.balance||(inv.amount-(inv.depositAmount||0))).toFixed(2)}</span>
                  </div>
                  <div style={{borderTop:"1px solid #F0E4D4",paddingTop:10,display:"flex",justifyContent:"space-between"}}>
                    <span style={{fontSize:14,fontWeight:800,color:"#2C1A0E"}}>Total</span>
                    <span style={{fontSize:22,fontWeight:800,color:primary}}>${inv.amount}</span>
                  </div>
                </>
              )}
            </div>
        ...
        {/* Mark as paid — shown when invoice is unpaid/overdue */}
        {inv.status !== "paid" && !markedPaid && (
          <div style={{marginBottom:16}}>
            {!showPaidConfirm ? (
              <button
                onClick={()=>setShowPaidConfirm(true)}
                style={{width:"100%",padding:"13px",borderRadius:12,border:"1.5px solid #D1ECE4",background:"transparent",color:"#155724",fontFamily:"sans-serif",fontWeight:700,fontSize:13,cursor:"pointer"}}
              >
                ✅ I've Sent Payment
              </button>
            ) : (
              <div style={{background:"#D1ECE4",borderRadius:14,padding:"16px",border:"1px solid #A3CFBB"}}>
                <div style={{fontSize:14,fontWeight:700,color:"#155724",marginBottom:6,textAlign:"center"}}>Confirm payment sent?</div>
                <div style={{fontSize:12,color:"#155724",marginBottom:12,textAlign:"center",opacity:.8}}>This will notify {storeName} that you've sent ${inv.amount}.</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
                  <button onClick={()=>setShowPaidConfirm(false)} style={{padding:"10px",borderRadius:10,border:"1px solid #A3CFBB",background:"transparent",color:"#155724",fontFamily:"sans-serif",fontWeight:600,fontSize:13,cursor:"pointer"}}>Cancel</button>
                  <button onClick={handleMarkPaid} disabled={markingPaid} style={{padding:"10px",borderRadius:10,border:"none",background:"#155724",color:"#fff",fontFamily:"sans-serif",fontWeight:700,fontSize:13,cursor:"pointer",opacity:markingPaid?.7:1}}>{markingPaid?"Confirming...":"Yes, Confirm"}</button>
                </div>
              </div>
            )}
          </div>
        ...
        {markedPaid && (
          <div style={{background:"#D1ECE4",borderRadius:14,padding:"20px",textAlign:"center",marginBottom:16}}>
            <div style={{fontSize:36,marginBottom:8}}>✅</div>
            <div style={{fontSize:16,fontWeight:800,color:"#155724"}}>Payment Confirmed!</div>
            <div style={{fontSize:13,color:"#155724",marginTop:4,opacity:.8}}>{storeName} has been notified.</div>
          </div>
        )}
...
function CustomerStorefrontPage({ products=[], categories=[], photos=[], albums=[], brand={}, socialLinks={}, bakerInfo={}, setPage, isOwnerPreview = false, bakerId="" }) {
  const [filterCat,  setFilterCat]  = useState("all");
  ...
  // Scroll to message form if URL has #bakery-message-form anchor
  useEffect(() => {
    if (window.location.hash === '#bakery-message-form') {
      setTimeout(() => {
        const el = document.getElementById('bakery-message-form');
  ...
  const usedCats  = categories.filter(c => activeProds.some(p => Array.isArray(p.categories) && p.categories.includes(c.id)));
  ...
  const filtered = filterCat === "all"
    ? activeProds
    : activeProds.filter(p => Array.isArray(p.categories) && p.categories.includes(filterCat));
...
function StorefrontPage({ products=[], categories=[], photos=[], bakerInfo, setBakerInfo, setPage, setBrand, bakerId="" }) {
  ...
  if(canvas){const a=document.createElement("a");a.download="bakeros-qr.png";a.href=canvas.toDataURL("image/png");a.click();show("QR downloaded ✓");}
        ...
        <div id="bakery-message-form" className="card" style={{padding:"16px",display:isOwnerPreview?"none":"block"}}>
          ...
          {canAccess(sfTier,"messages")
            ? <StorefrontMessageForm onSent={()=>show("Message sent! We'll be in touch soon ✓")} bakerPhone={bakerInfo?.phone||""} bakerId={bakerId}/>
        ...
        <div className="card" style={{padding:"14px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
          ...
          <div
            className="toggle"
            style={{background:safeBI.acceptingOrders===false?"#9CA3AF":"var(--p)"}}
            onClick={()=>setBakerInfo(p=>({...p, acceptingOrders: p.acceptingOrders===false ? true : false}))}
          >
            <div className="toggle-knob" style={{left:safeBI.acceptingOrders===false?3:23}}/>
          </div>
        </div>
...
function PaymentsPage({ payHandles, setPayHandles, invoices=[], orders=[] }){
  ...
  const isFree = pmtTier === "starter";

  // ── HANDLE INPUT SECTION (shown to ALL tiers) ────────────────────────────
  const handleInputsJSX = (
      ...
      <div style={{marginBottom:12}}>
        ...
        {localHandles.cashapp.trim() && (
          <div style={{marginTop:6,padding:"7px 11px",background:"#00D64F10",borderRadius:9,fontSize:11,color:"#16A34A",fontWeight:600}}>
            {isFree ? "🔒 Activates on invoices when you upgrade" : `Invoice link: cash.app/$${localHandles.cashapp.replace(/^\$/,"")}`}
          </div>
        )}
      </div>
      ...
      <div style={{marginBottom:12}}>
        ...
        {localHandles.venmo.trim() && (
          <div style={{marginTop:6,padding:"7px 11px",background:"#008CFF10",borderRadius:9,fontSize:11,color:"#008CFF",fontWeight:600}}>
            {isFree ? "🔒 Activates on invoices when you upgrade" : `Invoice link: venmo.com/u/${localHandles.venmo.replace(/^@/,"")}`}
          </div>
        )}
      </div>
      ...
      <div style={{marginBottom:14}}>
        ...
        {localHandles.zelle.trim() && (
          <div style={{marginTop:6,padding:"7px 11px",background:"#6D1ED410",borderRadius:9,fontSize:11,color:"#6D1ED4",fontWeight:600}}>
            {isFree ? "🔒 Activates on invoices when you upgrade" : `Customers send to: ${localHandles.zelle}`}
          </div>
        )}
      </div>

      <button className="pbtn" onClick={save} style={{width:"100%"}}>
        {isFree ? "Save My Handles" : "Save Payment Handles"}
      </button>

      {/* Trial upgrade prompt — shown after they add at least one handle */}
      {isFree && connectedCount > 0 && (
      ...
      <div style={{padding:"0 12px", display:"flex", flexDirection:"column", gap:12}}>
        ...
        {step===1 && (
          <>
            <Fld label="Full Name" required error={err.name}>
              <input className="field" placeholder="Jordan Smith" autoComplete="name" name="name" value={con.name} onChange={e=>setCon(p=>({...p,name:e.target.value}))}/>
            </Fld>
            <Fld label="Phone" required error={err.phone}>
              <input className="field" type="tel" placeholder="(210) 555-0100" autoComplete="tel" name="tel" value={con.phone} onChange={e=>setCon(p=>({...p,phone:e.target.value}))}/>
            </Fld>
            <Fld label="Email (optional)">
              <input className="field" type="email" placeholder="your@email.com" autoComplete="email" name="email" value={con.email} onChange={e=>setCon(p=>({...p,email:e.target.value}))}/>
            </Fld>
        ...
        {step===2 && (
          <>
            ...
            {items.length === 0 && (
              <div style={{background:`color-mix(in srgb,${primary} 8%,var(--bg))`,borderRadius:11,padding:"10px 13px",fontSize:12,color:primary,fontWeight:600,textAlign:"center",border:`1px solid ${primary}30`}}>
...
const PAGE_GUIDES = {
  ...
  advanced_crm: {
    icon: "🧠",
    title: "Advanced CRM",
    intro: "Pro-level customer management. Segment your audience, track NFC leads, set follow-up reminders, and see full order history per customer.",
    tips: [
      { emoji:"🏷", text:"Use tags to segment customers — VIP, Wholesale, Repeat, At Risk" },
      { emoji:"📲", text:"NFC captured leads appear here automatically when customers tap your tag" },
      { emoji:"⚠️", text:"Allergy flags are highlighted in red so you never miss a dietary restriction" },
      { emoji:"🔁", text:"Filter by lead source, tag, or order frequency to target the right customers" },
    ],
  },
  orders: {
    icon: "📋",
...
export default function App() {
    ...
    try {
      ...
      if (settings) {
        ...
        if (Object.keys(bi).length) {
          setBakerInfo({ bio:"", minOrder:"", deposit:"", leadTime:"", signatureItems:"", ...bi, flavors: Array.isArray(bi.flavors) ? bi.flavors : [] });
        }
      ...
      if (invoices?.length) {
        const mapped = invoices.map(i => ({
          ...
          items: i.items||"", email: i.email||"", phone: i.phone||"",
          depositAmount: parseFloat(i.deposit_amount)||0,
          balance: parseFloat(i.balance)||0,
          depositReceived: i.deposit_received||false,
        }));
        setInvoices(mapped);
      }
  ...
  const pushToSupabase = useCallback(async (userId, snapshot) => {
    // Safety check — never overwrite Supabase with an empty brand
    // This prevents data loss if the app loads before hydration completes
    if (!snapshot.brand?.storeName) {
      console.warn('[BakerOS] Push blocked — brand.storeName is empty, skipping to protect data');
      return;
    }
    // Additional guard — never sync if still hydrating
    if (isHydrating) {
      console.warn('[BakerOS] Push blocked — hydration in progress');
      return;
    }
  ...
  const queueSync = useCallback(() => {
    ...
    if (!hasHydrated.current) return; // never sync before first hydration completes
  ...
  // Cleanup pending sync timer on unmount to prevent stale writes
  useEffect(() => {
  ...
  const renderPageInner = () => {
    switch(page) {
      ...
      case "customers":   return <CustomersPage customers={customers} setCustomers={setCustomers} orders={appOrders}/>;
      case "crm":         return <CustomersPage customers={customers} setCustomers={setCustomers} orders={appOrders}/>; // legacy redirect
      case "advanced_crm": return <GateWall feature="crm_advanced"><AdvancedCRMPage customers={customers} setCustomers={setCustomers} orders={appOrders}/></GateWall>;
      ...
      case "storefront": {
        try {
          return <StorefrontPage products={products} categories={categories} photos={photos} bakerInfo={bakerInfo} setBakerInfo={setBakerInfo} setPage={setPage} setBrand={setBrand} bakerId={session?.user?.id || ""}/>;
        } catch(e) {
      ...
      case "payments":    return <PaymentsPage payHandles={payHandles} setPayHandles={setPayHandles} invoices={invoices} orders={appOrders}/>;
      case "receipt":     return <GateWall feature="receipt_scan"><ReceiptPage onAddToInventory={(items)=>setInventory(p=>[...p,...items])} inventory={inventory} setPage={setPage}/></GateWall>;
      ...
      case "orderform":   return <OrderFormPage products={products} photos={photos} bakerInfo={bakerInfo} addOrder={async(o)=>{
        ...
        // ── Send order confirmation email to customer if they have an email ──
        if (o.email) {
          const storeName = brand?.storeName || "Your Baker";
          const confirmHTML = `
            <div style="font-family:'Helvetica Neue',sans-serif;max-width:520px;margin:0 auto;background:#FFFBF5;border-radius:16px;overflow:hidden;border:1px solid #E8D5C0;">
              <div style="background:#C47B00;padding:28px 32px;text-align:center;">
                <div style="font-size:26px;font-weight:800;color:#fff;letter-spacing:-0.5px;">Order Received! 🎂</div>
                <div style="font-size:14px;color:rgba(255,255,255,0.85);margin-top:4px;">${storeName}</div>
              </div>
              <div style="padding:28px 32px;">
                <p style="font-size:15px;color:#2C1A0E;margin:0 0 8px;">Hi ${escapeHtml(o.customer)},</p>
                <p style="font-size:14px;color:#5C4A3A;line-height:1.6;margin:0 0 24px;">Thank you for your order! We've received it and will be in touch soon to confirm the details.</p>
                <div style="background:#FDF6EC;border-radius:12px;padding:20px;margin-bottom:24px;border:1px solid #E8D5C0;">
                  <div style="font-size:11px;font-weight:700;color:#9C7B5C;text-transform:uppercase;letter-spacing:1px;margin-bottom:14px;">Order Summary</div>
                  <div style="display:flex;justify-content:space-between;margin-bottom:10px;">
                    <span style="font-size:13px;color:#9C7B5C;">Item</span>
                    <span style="font-size:13px;font-weight:700;color:#2C1A0E;">${escapeHtml(o.item)}</span>
                  </div>
                  ${o.dueDate ? `<div style="display:flex;justify-content:space-between;margin-bottom:10px;">
                    <span style="font-size:13px;color:#9C7B5C;">Due Date</span>
                    <span style="font-size:13px;font-weight:700;color:#2C1A0E;">${escapeHtml(o.dueDate)}</span>
                  </div>` : ""}
                  ${o.notes ? `<div style="display:flex;justify-content:space-between;margin-bottom:10px;">
                    <span style="font-size:13px;color:#9C7B5C;">Notes</span>
                    <span style="font-size:13px;font-weight:700;color:#2C1A0E;text-align:right;max-width:60%;">${escapeHtml(o.notes)}</span>
                  </div>` : ""}
                  <div style="border-top:1px solid #E8D5C0;margin-top:12px;padding-top:12px;display:flex;justify-content:space-between;">
                    <span style="font-size:15px;font-weight:800;color:#2C1A0E;">Total</span>
                    <span style="font-size:20px;font-weight:800;color:#C47B00;">$${o.amount}</span>
                  </div>
                </div>
                ${bakerInfo?.deposit ? `<div style="background:#FFF8EC;border-radius:11px;padding:13px 16px;margin-bottom:20px;border:1px solid #E8D5C0;">
                  <div style="font-size:12px;font-weight:700;color:#9C7B5C;margin-bottom:4px;">💳 Deposit Policy</div>
                  <div style="font-size:13px;color:#5C4A3A;">${bakerInfo.deposit}</div>
                </div>` : ""}
                <div style="text-align:center;margin-bottom:20px;">
                  <a href="${`https://app.bakeros.app/store/${(brand?.bakeryUsername||(brand?.storeName||'bakery').toLowerCase().replace(/\s+/g,'-').replace(/[^a-z0-9-]/g,''))}#bakery-message-form`}" style="background:#C47B00;color:#fff;padding:14px 36px;border-radius:12px;text-decoration:none;font-weight:800;font-size:15px;display:inline-block;">💬 Message Bakery →</a>
                </div>
                <p style="font-size:13px;color:#9C7B5C;text-align:center;margin:0;">Tap the button above to send ${storeName} a message directly.</p>
              </div>
              <div style="background:#F0E4D4;padding:14px 24px;text-align:center;">
                <p style="color:#9C7B5C;font-size:11px;margin:0;">Powered by BakerOS · ${storeName}</p>
              </div>
            </div>`;
          ...
          apiFetch("/api/email", {
            ...
            body: JSON.stringify({
              to: [o.email],
              subject: `Order confirmed — ${o.item} · ${storeName}`,
              html: confirmHTML,
              from: `${storeName} <hello@bakeros.app>`,
              replyTo: bakerInfo?.email || undefined,
            })
          }).catch(()=>{});
        }
