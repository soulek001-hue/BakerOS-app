...
function MarketingPage({ setPage, bakerInfo = {}, products = [], customers = [] }){
  ...
  const CAMPAIGN_CATEGORIES = [
    {
      id:"sales", label:"Sales & Promotions", emoji:"🏷️",
      suggestions:[
    ...
    {
      id:"seasonal", label:"Seasonal & Holidays", emoji:"🎉",
      suggestions:[
    ...
    {
      id:"engage", label:"Customer Re-Engagement", emoji:"💌",
      suggestions:[
    ...
    {
      id:"emotional", label:"High-Converting Angles", emoji:"❤️",
      suggestions:[
    ...
    {
      id:"quick", label:"Quick Suggestions", emoji:"⚡",
      suggestions:[
  ...
  const [aiCategory, setAiCategory] = useState(null); // selected category
  ...
  const send = () => {
    ...
    if (compose === "email") {
      ...
      Promise.allSettled(sends).then(() => {
        setSending(false); setSent(true);
        show(`SMS campaign sent to ${targets.length} customer${targets.length!==1?"s":""} ✓`);
        setTimeout(() => { setSent(false); setCompose(null); setMsg(""); setSubj(""); }, 2500);
      });
    }
  };
  ...
  const generateCampaign = async () => {
    ...
    // Variation engine — pick a different angle each generation
    ...
    const thisHook = HOOKS[aiGenCount % HOOKS.length];
    const thisStructure = STRUCTURES[aiGenCount % STRUCTURES.length];
    const thisCTA = CTAS[aiGenCount % CTAS.length];
    const thisUrgency = URGENCY_STYLES[aiGenCount % URGENCY_STYLES.length];

    const systemPrompt = `You are an expert bakery marketing copywriter for ${BAKER_CTX.name}${BAKER_CTX.location ? ` in ${BAKER_CTX.location}` : ""}.
...
VARIATION RULES — you MUST follow these exactly:
- Hook style this generation: ${thisHook} (build the opening around this angle)
- Message structure: ${thisStructure}
- CTA style: use "${thisCTA}" or a close variation
- Urgency style: ${thisUrgency}
- NEVER start the SMS or email with "Hey", "Hi", "Hello", or any greeting used in a previous campaign
- NEVER use the phrase "fresh baked", "made with love", "handcrafted" more than once
- NEVER repeat a sentence structure or opening from a previous version
${aiPrevHooks.length > 0 ? `- Previous openings to AVOID starting with: ${aiPrevHooks.map(h=>`"${h}"`).join(", ")}` : ""}

Keep the selected tone (${aiStyle}) and audience (${aiAudience}) consistent. Reference specific products and flavors. Never sound corporate or generic.`;

    const userPrompt = `Write a marketing campaign:
Goal: ${aiGoal}
Audience: ${aiAudience}
Offer/Idea: ${aiOffer || "A special promotion"}
Tone: ${aiStyle}
Generation #${aiGenCount + 1} — make this structurally and creatively different from any previous version.

Return ONLY valid JSON (no markdown, no explanation):
{
  "sms": "SMS message (max 230 chars, end with [LINK], do NOT start with Hey/Hi/Hello)",
  "emailSubject": "email subject line (do not start with Re: or Fwd:)",
  "emailBody": "email body (3-4 paragraphs, end with [LINK], vary paragraph lengths)",
  "adHeadline": "ad headline (max 8 words, punchy)",
      ...
      <div style={{padding:"0 12px",display:"flex",flexDirection:"column",gap:12}}>
        ...
        >
          {aiLoading
            ...
            : aiResult
              ? <><span style={{fontSize:18}}>↺</span> Regenerate Campaign (v{aiGenCount+1})</>
