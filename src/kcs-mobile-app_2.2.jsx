...
function OrdersPage({ extraOrders=[], setAppOrders=()=>{}, customers=[], setCustomers=()=>{}, invoices=[], setInvoices=()=>{}, reminders=[], setReminders=()=>{}, tier="" }){
  ...
  const updS = (id, s) => {
    ...
    setAppOrders(p => p.map(o => o.id===id ? {...o, status:s} : o));
  ...
  return (
    <div style={{paddingBottom:96}}>
      ...
      <div style={{padding:"0 12px",display:"flex",flexDirection:"column",gap:11}}>
        ...
        {/* Stats */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {[["📨",String(campaigns?.length||0),"Campaigns sent"],["📬",String(customers.filter(c=>c.phone||c.email).length),"Reachable customers"]].map(([ic,val,lbl])=>(
...
function AccountingPage({inventory, orders=[], invoices=[]}){
  ...
  const connect=()=>{
    // QuickBooks OAuth integration — coming soon
    // For now, mark as connected so export works
    setConnecting(true);
...
async function parseReceipt(b64,cat){
  const res=await fetch("/api/claude",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({max_tokens:1000,messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:"image/jpeg",data:b64}},{type:"text",text:`Analyze this bakery receipt. Category: "${cat}". Return ONLY valid JSON: {"store":"name","date":"date","total":0,"items":[{"name":"clean name","qty":1,"unit":"unit","unitCost":0,"totalCost":0}]}. No tax/subtotal rows.`}]}]})});
  const data=await res.json();
...
function StorefrontMessageForm({ onSent, bakerPhone="" }) {
  ...
  const submit = () => {
    ...
    fetch("/api/notify",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:"message",customerName:form.name,item:`Message: ${form.subject||"Storefront inquiry"}`,amount:0,phone:form.phone,bakerPhone:bakerPhone||""})}).catch(()=>{});
  };
...
function clearAllData() {
  const keys = [
    ...
    "bos_seen_guides","bos_messages","bos_sms_templates","bos_recipes",
    ...
    switch(page) {
      ...
      case "orderform":   return <OrderFormPage products={products} photos={photos} bakerInfo={bakerInfo} addOrder={(o)=>{
        ...
        setCustomers(p=>{
          ...
          return [{id:Date.now(),name:o.customer,phone:o.phone,email:o.email||"",orders:1,spent:o.amount||0,last:"Today",tag:"New",smsOptIn:o.smsOptIn||false},...p];
        });
        ...
        fetch("/api/notify",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:"order",customerName:o.customer,item:o.item,amount:o.amount,phone:o.phone,bakerId:session?.user?.id||""})}).catch(()=>{});
      }}/>;
      ...
      <Ctx.Provider value={brand}>
        ...
        <div style={{position:"fixed",top:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,height:"100%",background:"var(--bg)",overflow:"hidden",display:"flex",flexDirection:"column",boxShadow:"0 0 60px rgba(0,0,0,.15)"}}>
          ...
          <div className="page-scroll" style={{flex:1,paddingTop:44,paddingBottom:inBottom?"calc(96px + env(safe-area-inset-bottom, 16px))":"96px",overflowY:"scroll",WebkitOverflowScrolling:"touch",overscrollBehaviorY:"contain",minHeight:0,height:0}}>
            ...
            {isHydrating && (
              <div style={{position:'fixed',inset:0,background:'#FDF6EC',zIndex:9999,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',pointerEvents:'none'}}>
