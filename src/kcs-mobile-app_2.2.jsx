...
function AdvancedCRMPage({ customers=[], setCustomers=()=>{}, orders=[] }) {
  ...
  const ALL_TAGS = ["all","VIP","Regular","New","Wholesale","Inactive","NFC Lead"];
  ...
  return (
    <div style={{paddingBottom:96}}>
      ...
      <div style={{padding:"0 12px 10px",display:"flex",flexDirection:"column",gap:8}}>
        <input className="field" placeholder="Search by name, phone, or email..." value={search} onChange={e=>setSearch(e.target.value)} style={{fontSize:13}}/>
      ...
      <div style={{padding:"0 12px",display:"flex",flexDirection:"column",gap:8}}>
        {filtered.length === 0 ? (
          <div className="card" style={{padding:"28px",textAlign:"center"}}>
            ...
            <div style={{fontSize:14,fontWeight:700,color:"var(--tx)",marginBottom:4}}>No customers match this filter</div>
            <div style={{fontSize:12,color:"var(--mu)"}}>Try a different search or tag filter</div>
          </div>
        ) : filtered.map(c => {
          ...
          return (
            <div key={c.id} className="card row" style={{padding:"12px 13px",cursor:"pointer"}} onClick={()=>setSel(c)}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3,flexWrap:"wrap"}}>
                    ...
                    {hasAllergy && <span style={{fontSize:9,fontWeight:700,background:"#FEE2E2",color:"#991B1B",borderRadius:20,padding:"1px 6px"}}>⚠️ Allergy</span>}
                  </div>
                  <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                    {allTags.slice(0,3).map(tag => {
                      const tc2 = TAGCOL[tag] || {bg:"var(--bd)",c:"var(--mu)"};
                      return <span key={tag} className="badge" style={{background:tc2.bg,color:tc2.c,fontSize:9}}>{tag}</span>;
                    })}
                  </div>
                  <div style={{fontSize:11,color:"var(--mu)",marginTop:3}}>
                    {selOrders.length} order{selOrders.length!==1?"s":""} · ${c.spent||0} · {c.last||"Never"}
                    {c.phone && ` · ${c.phone}`}
                  </div>
                </div>
      ...
      {/* Customer detail sheet — reuses the same full-detail sheet */}
      {sel && (()=>{
        const selOrders = orders.filter(o =>
          o.customer === sel.name ||
          (sel.phone && o.phone === sel.phone) ||
              ...
              <div style={{padding:"4px 16px calc(110px + env(safe-area-inset-bottom,0px))"}}>
                ...
                {(sel.phone||sel.email) && (
                  <div style={{background:"var(--bg)",borderRadius:11,padding:"11px 13px",marginBottom:10}}>
                    ...
                    {sel.phone&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:12,color:"var(--mu)"}}>Phone</span><span style={{fontSize:13,fontWeight:600,color:"var(--tx)"}}>{sel.phone}</span></div>}
                    {sel.email&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:12,color:"var(--mu)"}}>Email</span><span style={{fontSize:12,fontWeight:600,color:"var(--tx)",maxWidth:"60%",textAlign:"right",wordBreak:"break-all"}}>{sel.email}</span></div>}
                    {sel.source==="nfc"&&<div style={{marginTop:6}}><span style={{fontSize:10,fontWeight:700,background:"#EAF0FF",color:"#1A3A8F",borderRadius:20,padding:"2px 8px"}}>📲 NFC Lead</span></div>}
                    {sel.smsOptIn!==undefined&&<div style={{display:"flex",justifyContent:"space-between",marginTop:4}}><span style={{fontSize:12,color:"var(--mu)"}}>SMS opt-in</span><span style={{fontSize:12,fontWeight:700,color:sel.smsOptIn?"#16A34A":"#9CA3AF"}}>{sel.smsOptIn?"✓ Yes":"No"}</span></div>}
                  </div>
                )}
                {/* Allergies */}
                ...
                {/* Notes */}
                {(sel.note||sel.notes)&&<div style={{background:"var(--bg)",borderRadius:11,padding:"11px 13px",marginBottom:10}}><div style={{fontSize:10,fontWeight:700,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginBottom:5}}>📝 Notes</div><div style={{fontSize:13,color:"var(--tx)",lineHeight:1.6}}>{sel.note||sel.notes}</div></div>}
                {/* Order history */}
                {selOrders.length>0&&<div style={{background:"var(--bg)",borderRadius:11,padding:"11px 13px",marginBottom:10}}><div style={{fontSize:10,fontWeight:700,color:"var(--mu)",textTransform:"uppercase",letterSpacing:".8px",marginBottom:8}}>🎂 Order History</div><div style={{display:"flex",flexDirection:"column",gap:7}}>{selOrders.slice(0,5).map(o=><div key={o.id} style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",paddingBottom:7,borderBottom:"1px solid var(--bd)"}}><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:"var(--tx)"}}>{o.item}</div><div style={{fontSize:11,color:"var(--mu)"}}>{o.date}{o.dueDate?` · Due ${o.dueDate}`:""}</div></div><div style={{textAlign:"right",marginLeft:10}}><div style={{fontSize:13,fontWeight:800,color:"var(--p)"}}>${o.amount||0}</div><div style={{fontSize:10,color:"var(--mu)",textTransform:"capitalize"}}>{o.status}</div></div></div>)}{selOrders.length>5&&<div style={{fontSize:11,color:"var(--mu)",textAlign:"center"}}>+{selOrders.length-5} more</div>}</div></div>}
                {/* Actions */}
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
