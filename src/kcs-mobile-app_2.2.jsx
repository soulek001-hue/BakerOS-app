...
const FEATURES = {
  ...
  crm_notes:               ["pro","elite"],
  crm_tags:                ["pro","elite"],
  crm_allergy_flags:       ["pro","elite"],
  crm_search:              ["pro","elite"],
...
function getStorefrontSlug(brand, bakerInfo) {
  ...
  return name.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"") || "your-store";
}
...
const BAKEROS_LOGO = "/bakeros-logo.png";
...
function SparkBar({orders=[]}){
  ...
  const sales=days.map((_,i)=>{
    ...
    return orders.filter(o=>{
      ...
      const d=new Date(o.completedAt||o.date||0);
  ...
  if(!hasData)return(
    <div style={{height:52,display:"flex",alignItems:"center",justifyContent:"center",color:"var(--mu)",fontSize:11,textAlign:"center"}}>
      Complete orders to see weekly revenue
    </div>
  );
  return(
    <div style={{display:"flex",alignItems:"flex-end",gap:4,height:52}}>
      {sales.map((v,i)=>(
        <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
          <div style={{width:"100%",background:i===todayIdx?"var(--p)":"var(--bd)",height:`${(v/mx)*44}px`,borderRadius:"2px 2px 0 0",minHeight:v>0?2:0,opacity:i===todayIdx?1:.7}}/>
          <span style={{fontSize:7,color:"var(--mu)",fontWeight:600}}>{days[i].slice(0,3)}</span>
        </div>
      ))}
    </div>
  );
      ...
      <div style={{position:"absolute",left:0,top:0,bottom:0,width:280,background:"var(--sf)",boxShadow:"4px 0 32px rgba(0,0,0,.25)",animation:"drawerIn .28s cubic-bezier(.22,.68,0,1.1) both",display:"flex",flexDirection:"column",overflowY:"auto"}}>
        ...
        <div style={{flex:1,padding:"8px 10px"}}>
          {DMORE.map(item=>{
            ...
            return(
              <button key={item.id} onClick={()=>{setPage(item.id);onClose();}} style={{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"11px 12px",borderRadius:11,border:"none",background:act?`color-mix(in srgb,${b.theme.primary} 12%,var(--sf))`:"transparent",cursor:"pointer",fontFamily:"var(--fb)",marginBottom:2}}>
    ...
    <div style={{paddingBottom:96}}>
      ...
      <div style={{padding:"12px 12px 0"}}>
        <div className="card" style={{padding:"14px"}}>
          ...
          <SparkBar orders={completedOrders}/>
        </div>
      </div>
      {/* Recent orders */}
      <div style={{padding:"12px 12px 0"}}>
...
function OrdersPage({ extraOrders=[], setAppOrders=()=>{}, customers=[], setCustomers=()=>{}, invoices=[], setInvoices=()=>{}, reminders=[], setReminders=()=>{}, tier="" }){
  ...
  const updS = (id, s) => {
    ...
    setAppOrders(p => p.map(o => {
      if (o.id !== id) return o;
      const patch = { ...o, status: s };
      if (s === "completed" && !o.completedAt) patch.completedAt = new Date().toISOString();
      return patch;
    }));
    setSel(p => p ? {...p, status:s, ...(s==="completed"&&!p.completedAt?{completedAt:new Date().toISOString()}:{})} : p);
    show(`Status → ${s} ✓`);
    if (s === "completed") {
  ...
  return (
    <div style={{paddingBottom:96}}>
      ...
      <div style={{padding:"0 12px",display:"flex",flexDirection:"column",gap:9}}>
        {allOrders.length===0&&<div style={{padding:"48px 24px",textAlign:"center"}}>
          <div style={{fontSize:48,marginBottom:12}}>📋</div>
          <div style={{fontSize:15,fontWeight:700,color:"var(--tx)",marginBottom:6}}>No orders yet</div>
          <div style={{fontSize:12,color:"var(--mu)",lineHeight:1.5}}>Orders will appear here once customers submit via your storefront link</div>
        </div>}
...
/* ── MARKETING ─────────────────────────────── */
function MarketingPage({ setPage, bakerInfo = {}, products = [], customers = [] }){
  const b      = useBrand();
  ...
  const send = () => {
    if (!msg.trim()) return;
    setSending(true);
    // SMS campaigns queued — delivery pending Twilio A2P 10DLC registration
    const targets = customers.filter(c => c.phone);
    const sends = targets.map(c =>
      fetch("/api/notify",{
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({customerName:c.name, item:"Marketing Campaign", amount:0, phone:c.phone, customMessage:msg, bakerPhone:bakerInfo?.phone||""})
      }).catch(()=>{})
    );
    Promise.allSettled(sends).then(() => {
      setSending(false); setSent(true);
      show(`SMS campaign queued for ${targets.length} customer${targets.length!==1?"s":""} — delivery pending A2P registration`);
      setTimeout(() => { setSent(false); setCompose(null); setMsg(""); setSubj(""); }, 3000);
    });
  };
  ...
  if (compose) return (
    <div style={{paddingBottom:96}}>
      ...
      {sent
        ...
        : <div style={{padding:"0 16px",display:"flex",flexDirection:"column",gap:13}}>
            {compose==="sms" && (
              <div style={{background:"#FEF3C7",border:"1.5px solid #D97706",borderRadius:10,padding:"10px 14px",fontSize:12,color:"#92400E",display:"flex",alignItems:"flex-start",gap:8}}>
                <span style={{fontSize:16,flexShrink:0}}>📱</span>
                <div><strong>SMS delivery is paused</strong> — Twilio A2P 10DLC registration pending. Campaigns are saved but not delivered to phones yet. <strong>Email campaigns work normally.</strong></div>
              </div>
            )}
            ...
            {canSend&&<button className="pbtn" onClick={send} disabled={!msg||sending} style={{width:"100%"}}>{sending?"Sending...":`Send to ${b.storeName ? "your customers" : "248"} 🚀`}</button>}
          </div>
      }
    </div>
  );
  ...
  return (
    <div style={{paddingBottom:96}}>
      ...
      <div style={{padding:"0 12px",display:"flex",flexDirection:"column",gap:11}}>
        ...
        {/* Stats */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          <div className="card" style={{padding:"11px 13px",gridColumn:"1/-1",textAlign:"center",color:"var(--mu)"}}>
            <div style={{fontSize:22,marginBottom:6}}>📊</div>
            <div style={{fontSize:13,fontWeight:700,color:"var(--tx)",marginBottom:3}}>No campaign data yet</div>
            <div style={{fontSize:11,lineHeight:1.5}}>Send your first campaign to see open rates, reach, and conversions here.</div>
          </div>
        </div>
...
/* ── OTHER PAGES ───────────────────────────── */
function CRMPage({ customers=CUSTS_INIT, setCustomers=()=>{} }){
  const b = useBrand();
  ...
  const saveCustomer = () => {
    ...
    const isStarterCRM = !canAccess(crmTier,"unlimited_products");
    if (isStarterCRM && customers.length >= 50) { show("Trial Access allows up to 50 customers. Upgrade to Growth for unlimited.","error"); return; }
    const newC = {
      id: Date.now(),
      name: form.name.trim(),
...
function AnalyticsPage({ orders=[], customers=[] }){
  ...
  return <div style={{paddingBottom:96}}><PH title="Analytics" sub="Business performance"/><div style={{padding:"0 12px",display:"flex",flexDirection:"column",gap:11}}>
    ...
    <div className="card" style={{padding:"13px"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><span style={{fontSize:14,fontWeight:700,color:"var(--tx)"}}>Weekly Revenue</span><span style={{fontSize:12,color:"var(--mu)"}}>${(()=>{const w=Date.now()-7*86400000;return (orders||[]).filter(o=>o.status==="completed"&&new Date(o.completedAt||0)>=w).reduce((s,o)=>s+(o.amount||0),0).toLocaleString();})()}</span></div><SparkBar orders={orders}/></div>
...
function CalendarPage({ orders=[], reminders=[], setReminders=()=>{} }) {
  ...
  const HOLIDAYS = useMemo(() => [
    ...
    { date:"2026-02-14", name:"Valentine's Day",           emoji:"❤️", color:"#DC2626", idea:`Heart-shaped cakes, custom cookie boxes, and couples treats are huge sellers in ${cityName}. Start promoting 3 weeks out! Offer a 'Build Your Own Valentine Box' for a personal touch.` },
    ...
    { date:"2027-02-14", name:"Valentine's Day",           emoji:"❤️", color:"#DC2626", idea:`Heart-shaped cakes, custom cookie boxes, and couples treats are huge in ${cityName}. Start promoting 3 weeks out! Offer a 'Build Your Own Valentine Box' for a personal touch.` },
...
/* ── PRODUCTS PAGE ─────────────────────────── */
function ProductFormSheet({ isEdit, onClose, productForm, setProductForm, categories, products, SELL_BY_OPTIONS, MIN_ORDER_OPTIONS, openAddCat, toggleProductCat, catColor, catEmoji, catName, editingProduct, saveProduct, productImgRef, deleteProduct }) {
  const b = useBrand();
...
function ProductsPage({ products, setProducts, categories, setCategories, tier="starter" }) {
  const b = useBrand();
  const [t, show] = useToast();
          ...
          <div className="sheet-panel">
            ...
            <div style={{padding:"12px 16px",display:"flex",flexDirection:"column",gap:9}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
                ...
                <button
                  onClick={()=>{ if(!window.confirm(`Delete "${viewPhoto?.title || 'this photo'}"? This cannot be undone.`)) return; deletePhoto(viewPhoto.id); }}
...
const MESSAGES_STORE = {
  ...
  listeners: [],
  _load() {
    if (this._msgs) return this._msgs;
    try { const s=localStorage.getItem(this._key); this._msgs=s?JSON.parse(s):MESSAGES_INIT; } catch { this._msgs=MESSAGES_INIT; }
    return this._msgs;
  },
  _save() { try { localStorage.setItem(this._key,JSON.stringify(this._msgs)); } catch {} },
  get() { return this._load(); },
  add(msg) { this._load(); this._msgs=[msg,...this._msgs]; this._save(); this.listeners.forEach(l=>l([...this._msgs])); },
  markRead(id) { this._load(); this._msgs=this._msgs.map(m=>m.id===id?{...m,read:true}:m); this._save(); this.listeners.forEach(l=>l([...this._msgs])); },
  markReplied(id) { this._load(); this._msgs=this._msgs.map(m=>m.id===id?{...m,replied:true,read:true}:m); this._save(); this.listeners.forEach(l=>l([...this._msgs])); },
  update(id,patch) { this._load(); this._msgs=this._msgs.map(m=>m.id===id?{...m,...patch}:m); this._save(); this.listeners.forEach(l=>l([...this._msgs])); },
  subscribe(fn) { this.listeners.push(fn); return ()=>{ this.listeners=this.listeners.filter(l=>l!==fn); }; },
};
...
function StorefrontMessageForm({ onSent }) {
  ...
  const submit = () => {
    ...
    setSent(true);
    onSent && onSent();
    // Notify baker via SMS that a new message arrived
    // bakerPhone is passed via the storefront data loaded from Supabase
    const bPhone = window.__bakerPhone || "";
    fetch("/api/notify",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({customerName:form.name,item:`Message: ${form.subject||"Storefront inquiry"}`,amount:0,phone:form.phone,bakerPhone:bPhone})}).catch(()=>{});
  };
...
function MessagesPage() {
  ...
  const [msgView, setMsgView] = useState("inbox"); // "inbox" | "archive"
  const [t, show] = useToast();
          ...
          <div className="sheet-panel">
            ...
            <div style={{padding:"4px 16px 0"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
                ...
                <button className="gbtn" onClick={()=>{
                  MESSAGES_STORE.update(sel.id, {archived:!sel.archived});
...
function CustomerStorefrontPage({ products=[], categories=[], photos=[], brand={}, socialLinks={}, bakerInfo={}, setPage, isOwnerPreview = false }) {
  const [filterCat,  setFilterCat]  = useState("all");
    ...
    <div style={{paddingBottom:96}}>
      ...
      <div style={{padding:"0 12px",display:"flex",flexDirection:"column",gap:11}}>
        {(()=>{
          ...
          const stats=[["💰","$"+paid.toLocaleString(),"Collected"],["⏳","$"+pending.toLocaleString(),"Pending"],["📈","$"+avg,"Avg Order"],["🔄","$0","Refunds"]];
...
function SettingsPage({ socialLinks, setSocialLinks, brand, setBakerInfo, bakerInfo, setOnboarded, setPage, obBakery }){
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      if (session?.user?.id) {
        await supabase.from('baker_data').delete().eq('user_id', session.user.id);
      }
    } catch(e) { console.warn('Reset: could not delete from Supabase', e.message); }
    clearAllData();
    MESSAGES_STORE._msgs = null;
      ...
      <div style={{padding:"0 12px",display:"flex",flexDirection:"column",gap:13}}>
        ...
        <div className="card" style={{padding:"15px"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:11}}>
            ...
            <button className="pbtn" onClick={()=>{setBakerInfo&&setBakerInfo(p=>p); show&&show("Account info saved ✓");}} style={{width:"100%",marginTop:10}}>Save Account Info</button>
          </div>
        </div>
        ...
        <div className="card" style={{padding:"15px",border:"1px solid #FCA5A530"}}>
          ...
          <button
            onClick={async()=>{ await supabase.auth.signOut(); clearAllData(); MESSAGES_STORE._msgs=null; window.location.reload(); }}
          ...
          {!showReset ? (
            ...
            <div style={{display:"flex",flexDirection:"column",gap:9}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
                ...
                <button onClick={async()=>{ try{ const{data:{session}}=await supabase.auth.getSession(); if(session?.user?.id) await supabase.from('baker_data').delete().eq('user_id',session.user.id); }catch(e){} clearAllData(); MESSAGES_STORE._msgs=null; await supabase.auth.signOut(); window.location.reload(); }} style={{padding:"11px",borderRadius:11,border:"none",background:"#DC2626",color:"#fff",fontFamily:"var(--fb)",fontWeight:700,fontSize:13,cursor:"pointer"}}>
                  Yes, Reset
                </button>
              </div>
            </div>
...
function OrderFormPage({ products=[], photos=[], addOrder, bakerInfo={} }){
  ...
  const [con,     setCon]        = useState({name:"", phone:"", email:"", dueDate:"", notes:""});
  ...
  const validate = () => {
    ...
    if (step===1 && !con.phone && !con.email) e.contact = "Phone or email required";
    if (step===1 && con.phone && !phoneRegex.test(con.phone.replace(/\s/g,""))) e.phone = "Enter a valid US phone number";
...
function OnboardingFlow({ step, setStep, bakery, setBakery, products, setProducts, errors, setErrors, linkCopied, setLinkCopied, brand, tier, socialLinks, setSocialLinks, onComplete }) {
  ...
  const next = async () => {
    ...
    if (step === 6) {
      ...
      if (!supabase) { setErrors({ email: "* Auth not ready, please refresh and try again." }); return; }
...
function clearAllData() {
  const keys = [
    ...
    "bos_seen_guides","bos_messages","bos_sms_templates",
...
export default function App() {
  ...
  const hydrateFromSupabase = useCallback(async (userId) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .eq('user_id', userId)
        .single();
      if (error || !data) { setIsHydrating(false); return; }
      const d = data.payload;
      if (d.brand)       setBrand(prev => ({ ...DEFAULT_BRAND, ...prev, ...d.brand, theme: d.brand.theme || (prev && prev.theme) || DEFAULT_BRAND.theme }));
      if (d.bakerInfo)   setBakerInfo(prev => ({ bio:"", minOrder:"", deposit:"", leadTime:"", flavors:[], signatureItems:"", ...prev, ...d.bakerInfo, flavors: Array.isArray(d.bakerInfo.flavors) ? d.bakerInfo.flavors : (prev?.flavors||[]) }));
      if (d.socialLinks) setSocialLinks(d.socialLinks);
      if (d.payHandles)  setPayHandles(d.payHandles);
      if (d.products)    setProducts(d.products);
      if (d.categories)  setCategories(d.categories);
      if (d.inventory)   setInventory(d.inventory);
      if (d.photos)      setPhotos(d.photos);
      if (d.orders)      setAppOrders(d.orders);
      ...
      if (d.messages) {
        try { localStorage.setItem('bos_messages', JSON.stringify(d.messages)); MESSAGES_STORE._msgs = null; } catch {}
      }
      localStorage.setItem('bos_last_synced', new Date().toISOString());
      console.log('[BakerOS] Hydrated from Supabase ✓');
    } catch (e) { console.warn('[BakerOS] Hydration failed:', e.message); }
    setIsHydrating(false);
  }, [setBrand, setBakerInfo, setSocialLinks, setPayHandles, setProducts, setCategories, setInventory, setPhotos, setAppOrders, setCustomers, setInvoices, setReminders, setTier, setOnboarded]);

  const pushToSupabase = useCallback(async (userId, snapshot) => {
    setSyncStatus('saving');
    try {
      const { data: current } = await supabase
        .from('baker_data').select('version').eq('user_id', userId).single();
      const serverVersion = current?.version || 0;
      if (serverVersion > (snapshot.version || 0) + 1) {
        setSyncStatus('conflict');
        setConflictBanner(true);
        return;
      }
      await supabase.from('baker_data').upsert({
        user_id: userId,
        updated_at: new Date().toISOString(),
        version: serverVersion + 1,
        payload: { ...snapshot, version: serverVersion + 1 },
      }, { onConflict: 'user_id' });
      setSyncStatus('saved');
  ...
  const queueSync = useCallback(() => {
    ...
    syncTimer.current = setTimeout(() => {
      const snapshot = {
        ...
        products, categories, inventory, photos,
        orders: appOrders, customers, invoices, reminders,
        tier, onboarded,
        messages: MESSAGES_STORE.get(),
      };
      pushToSupabase(session.user.id, snapshot);
    }, 5000); // 5s debounce
  }, [session, isHydrating, brand, bakerInfo, socialLinks, payHandles, products, categories, inventory, photos, appOrders, customers, invoices, reminders, tier, onboarded, pushToSupabase]);
  ...
  const completeOnboarding = useCallback(() => {
    ...
    setTooltipStep(1);
  }, [obBakery, obProducts, track, setSocialLinks]);
  ...
  const [publicLoading, setPublicLoading] = useState(!!urlSlug);
  const [publicError,  setPublicError]  = useState(null);

  useEffect(() => {
    if (!urlSlug) return;
    fetch(`/api/storefront?slug=${encodeURIComponent(urlSlug)}`)
      .then(r => r.json())
      .then(d => {
        if (d.error) setPublicError(d.error);
        else { setPublicData(d); window.__bakerPhone = d.bakerInfo?.phone || ""; }
      })
  ...
  const renderPage = () => {
    switch(page) {
      ...
      case "marketing":   return <MarketingPage setPage={setPage} bakerInfo={bakerInfo} products={products} customers={customers}/>;
      ...
      case "crm":         return <GateWall feature="crm"><CRMPage customers={customers} setCustomers={setCustomers}/></GateWall>;
      ...
      case "storefront": {
        try {
          return <StorefrontPage products={products} categories={categories} photos={photos} bakerInfo={bakerInfo} setBakerInfo={setBakerInfo} setPage={setPage}/>;
        } catch(e) {
      ...
      case "customer_storefront": return <CustomerStorefrontPage products={products} categories={categories} photos={photos} brand={brand} socialLinks={socialLinks} bakerInfo={bakerInfo} setPage={setPage} isOwnerPreview={true}/>;
      ...
      case "products":    return <ProductsPage products={products} setProducts={setProducts} categories={categories} setCategories={setCategories} tier={tier}/>;
      ...
      case "orderform":   return <OrderFormPage products={products} photos={photos} bakerInfo={bakerInfo} addOrder={(o)=>{
        setAppOrders(p=>[o,...p]);
        // Auto-add customer if not already in CRM
        setCustomers(p=>{
          const exists=p.find(c=>c.phone===o.phone||c.name===o.customer);
          if(exists){
            return p.map(c=>(c.phone===o.phone||c.name===o.customer)?{...c,orders:(c.orders||0)+1,last:"Today"}:c);
          }
          return [{id:Date.now(),name:o.customer,phone:o.phone,email:o.email||"",orders:1,spent:o.amount||0,last:"Today",tag:"New"},...p];
        });
        ...
        // Notify baker via SMS
        fetch("/api/notify",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({customerName:o.customer,item:o.item,amount:o.amount,phone:o.phone,bakerPhone:bakerInfo?.phone||""})}).catch(()=>{});
      }}/>;
            ...
            <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:9}}>
              ...
              {conflictBanner && (
                <div style={{position:"fixed",top:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:"#D97706",color:"#fff",padding:"10px 16px",fontSize:12,fontWeight:700,zIndex:9998,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ⚠ Another device updated your data. Refresh to get latest.
                  <button onClick={()=>window.location.reload()} style={{background:"rgba(255,255,255,.25)",border:"none",color:"#fff",borderRadius:6,padding:"4px 10px",cursor:"pointer",fontWeight:700}}>Refresh</button>
                </div>
              )}
