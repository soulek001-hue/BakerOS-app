...
// Store name + city fields for all tiers — uses brand context directly
function StorefrontNameFields() {
  const ctx = useContext(Ctx);
  if (!ctx) return null;
  const { brand, setBrand } = ctx;
  const [saved, setSaved] = useState(false);
  const storeName = (brand?.storeName === "BakerOS" || brand?.storeName === "My Bakery") ? "" : (brand?.storeName || "");
  const city = brand?.city && brand?.state ? `${brand.city}, ${brand.state}` : (brand?.city || "");
  const [localName, setLocalName] = useState(storeName);
  const [localCity, setLocalCity] = useState(city);

  const handleSave = () => {
    const parts = localCity.split(",").map(s => s.trim());
    setBrand(p => ({...p, storeName: localName, city: parts[0] || "", state: parts[1] || ""}));
    setSaved(true);
