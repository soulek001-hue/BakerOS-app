  ...
  const hydrateFromSupabase = useCallback(async (userId) => {
    ...
    try {
      ...
      if (settings) {
        ...
        if (Object.keys(b).length) setBrand(prev => ({ ...DEFAULT_BRAND, ...prev, ...b, theme: b.theme || (prev && prev.theme) || DEFAULT_BRAND.theme }));
        const bi = settings.baker_info || {};
        if (Object.keys(bi).length) setBakerInfo(prev => ({ bio:"", minOrder:"", deposit:"", leadTime:"", flavors:[], signatureItems:"", ...prev, ...bi, flavors: Array.isArray(bi.flavors) ? bi.flavors : (prev?.flavors||[]) }));
      ...
      if (customers?.length) {
        const mapped = customers.map(c => ({
          ...
          tag: c.tag, notes: c.notes, smsOptIn: c.sms_opt_in, allergies: c.allergies,
        }));
        setCustomers(mapped);
      }
      ...
      case "orderform":   return <OrderFormPage products={products} photos={photos} bakerInfo={bakerInfo} addOrder={async(o)=>{
        ...
        if (userId) {
          ...
          const { data: existing } = await supabase.from("baker_customers")
            ...
            .or(`phone.eq.${o.phone||""},name.eq.${o.customer}`).single();
          if (existing) {
        ...
        fetch("/api/notify",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:"order",customerName:o.customer,item:o.item,amount:o.amount,phone:o.phone,bakerId:userId||""})}).catch(()=>{});
      }}/>;
