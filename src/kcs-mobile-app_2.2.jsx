...
async function parseReceipt(b64,cat,userId=null){
  const res=await fetch("/api/claude",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({feature:"receipt_scan",userId,max_tokens:1000,messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:"image/jpeg",data:b64}},{type:"text",text:`Analyze this bakery receipt. Category: "${cat}". Return ONLY valid JSON: {"store":"name","date":"date","total":0,"items":[{"name":"clean name","qty":1,"unit":"unit","unitCost":0,"totalCost":0}]}. No tax/subtotal rows.`}]}]})});
  const data=await res.json();
  ...
  if(canvas){const a=document.createElement("a");a.download="bakeros-qr.png";a.href=canvas.toDataURL("image/png");a.click();show("QR downloaded ✓");}
        ...
        <div style={{background:"#F5F0FF",border:"1.5px solid #7C5CBF30",borderRadius:14,padding:"16px"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:12}}>
            {["NFC tap opens a lead capture form","Name, phone & email auto-save to CRM","Unlocks SMS & email marketing follow-up","Hardware NFC tag programming coming soon"].map(f=>(
    ...
    <div style={{paddingBottom:96}}>
      ...
      <div style={{padding:"0 12px",display:"flex",flexDirection:"column",gap:11}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
          {[["👥","Total",leads.length],["🔥","Hot",leads.filter(l=>l.tag==="Hot").length],["✅","Saved",leads.length]].map(([ic,lbl,val])=>(
        ...
        {/* NFC hardware coming soon banner */}
        <div style={{background:"#FEF3C7",border:"1.5px solid #D97706",borderRadius:13,padding:"12px 14px",display:"flex",gap:10,alignItems:"flex-start"}}>
          <span style={{fontSize:16,flexShrink:0}}>🔧</span>
          <div>
            <div style={{fontSize:12,fontWeight:800,color:"#92400E",marginBottom:2}}>NFC hardware writing coming soon</div>
            <div style={{fontSize:11,color:"#78350F",lineHeight:1.5}}>Physical NFC tag programming is in development. For now, use the simulator below to preview the lead capture experience your customers will see.</div>
          </div>
        </div>
        {/* How it works */}
