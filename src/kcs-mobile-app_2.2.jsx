...
export default function App() {
  ...
  const hydrateFromSupabase = useCallback(async (userId) => {
    ...
    try {
      ...
      if (d.messages) {
        // messages now in baker_messages table, not blob
      }
  ...
  const pushToSupabase = useCallback(async (userId, snapshot) => {
    ...
    try {
      // Version conflict check — detect if another device has newer data
      const { data: current } = await supabase.from('baker_data').select('updated_at').eq('user_id', userId).single();
      if (current?.updated_at && snapshot._lastSyncedAt) {
        const serverTime = new Date(current.updated_at).getTime();
        const ourSyncTime = new Date(snapshot._lastSyncedAt).getTime();
        if (serverTime > ourSyncTime + 5000) { // 5s grace period
  ...
  const queueSync = useCallback(() => {
    ...
    syncTimer.current = setTimeout(() => {
      const snapshot = {
        ...
        // messages now stored in baker_messages Supabase table, not blob
      };
            ...
            <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:9}}>
              ...
              {conflictBanner && (
                <div style={{position:"fixed",top:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:"#D97706",color:"#fff",padding:"10px 16px",fontSize:12,fontWeight:700,zIndex:9998,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  <button onClick={()=>window.location.reload()} style={{background:"rgba(255,255,255,.25)",border:"none",color:"#fff",borderRadius:6,padding:"4px 10px",cursor:"pointer",fontWeight:700,fontFamily:"var(--fb)"}}>Refresh</button>
                </div>
              )}
