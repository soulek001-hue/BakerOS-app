...
function NFCPage({ bakerInfo, brand, customers=[] }){
  ...
  const hotLeads = nfcLeads.filter(c => c.tag==='NFC Lead' || (c.tags||[]).includes('NFC Lead'));
...
function NFCLandingPage({ data, slug }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaved(true);
      // Redirect to storefront after 1.5s
  ...
  const handleSkip = () => { window.location.href = storefrontURL; };
    ...
    <div style={{minHeight:"100vh",background:bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 20px",fontFamily:"'Outfit',sans-serif",boxSizing:"border-box"}}>
      ...
      {saved
        ...
        : <div style={{width:"100%",maxWidth:360,display:"flex",flexDirection:"column",gap:14}}>
            ...
            <button
              onClick={handleSkip}
              style={{background:"transparent",border:"none",color:"#9C7B5C",fontFamily:"inherit",fontSize:13,cursor:"pointer",padding:"4px",textDecoration:"underline"}}
            >
              Skip, just browse
            </button>
          </div>
...
export default function App() {
  ...
  const [publicLoading, setPublicLoading] = useState(!!urlSlug || !!nfcSlug);
  ...
  const renderPageInner = () => {
    switch(page) {
      ...
      case "nfc":         return <NFCPage bakerInfo={bakerInfo} brand={brand} customers={appCustomers}/>;
      case "storefront": {
        try {
    ...
    <TierCtx.Provider value={tierCtxValue}>
      <Ctx.Provider value={brand}>
        ...
        <div style={{position:"fixed",top:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,height:"100%",background:"var(--bg)",overflow:"hidden",display:"flex",flexDirection:"column",boxShadow:"0 0 60px rgba(0,0,0,.15)"}}>
          ...
          <Drawer open={drawerOpen} onClose={()=>setDrawer(false)} setPage={setPage} page={page} tier={tier} nfcNewCount={appCustomers.filter(c=>c.isNewNfc||c.is_new_nfc).length}/>
